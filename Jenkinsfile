            ...
            steps {
                configFileProvider([configFile(fileId: 'maven-settings-with-deploy-snapshot', variable: 'MAVEN_SETTINGS')]) {
                    script {
                        ...
                        sh "${mvnHome}/bin/mvn clean install -pl :infinispan-core -am -PtraceTests -s $MAVEN_SETTINGS -Dmaven.test.failure.ignore=true || true"
                        junit testDataPublishers: [[$class: 'ClaimTestDataPublisher']], testResults: '**/target/*-reports/*.xml'
                        archive includes '**/infinispan*log.gz'
                    }
                }
            }
        }
