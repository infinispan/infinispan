...
public class MemoryMonitorTest {
   ...
   @Test
   public void testGcDurationAlert() {
      monitor.setGcDurationThreshold(100);
      assertFalse(monitor.isGcDurationExceeded());

      // Event below threshold
      monitor.recordGcEvent(1000, 50);
      assertFalse(monitor.isGcDurationExceeded());

      // Event at threshold
      monitor.recordGcEvent(2000, 100);
      assertTrue(monitor.isGcDurationExceeded());

      // Reset clears the alert
      monitor.reset();
      assertFalse(monitor.isGcDurationExceeded());
   }
   ...
   public void testResetClearsAllAlerts() {
      monitor.setGcDurationThreshold(100);
      monitor.recordGcEvent(1000, 200);
      assertTrue(monitor.isGcDurationExceeded());
   ...
   @Test
   public void testSetThresholdResetsAlert() {
      monitor.setGcDurationThreshold(100);
      monitor.recordGcEvent(1000, 200);
      assertTrue(monitor.isGcDurationExceeded());

      // Changing threshold resets the alert
      monitor.setGcDurationThreshold(500);
      assertFalse(monitor.isGcDurationExceeded());
   }
}
