...
public class MurmurHash3 implements Hash {
   ...
   // todo the hashFn is not matching the class name
   private final static LongHashFunction hashFn = LongHashFunction.xx();
   ...
   public static int hash(byte[] payload, int seed) {
      return (int) LongHashFunction.xx(seed).hashBytes(payload);
   }

   public static int hash(long[] payload, int seed) {
      return (int) LongHashFunction.xx(seed).hashLongs(payload);
   }
