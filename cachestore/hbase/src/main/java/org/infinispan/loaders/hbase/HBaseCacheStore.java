...
public class HBaseCacheStore extends AbstractCacheStore {
      ...
      if (config.autoCreateTable) {
         ...
         try {
            ...
            if (ex.getCause() instanceof TableExistsException) {
               log.info("Not creating " + this.entryTable + " because it already exists.");
            } else {
         ...
         try {
            List<String> colFamilies = new ArrayList<String>();
            colFamilies.add(this.expirationColumnFamily);
            // column family should only support a max of 1 version
            hbf.createTable(this.expirationTable, colFamilies, 1);
         } catch (HBaseException ex) {
            if (ex.getCause() instanceof TableExistsException) {
               log.info("Not creating " + this.expirationTable + " because it already exists.");
            } else {
               throw new CacheLoaderException("Got HadoopException while creating the "
   ...
   public void store(InternalCacheEntry entry) throws CacheLoaderException {
      log.debug("In HBaseCacheStore.store for " + this.entryTable + ": " + entry.getKey());
      ...
      try {
         ...
         valMap.put(entryValueField, val);

         Map<String, Map<String, byte[]>> cfMap = new HashMap<String, Map<String, byte[]>>();
         cfMap.put(entryColumnFamily, valMap);
         ...
         if (entry.canExpire()) {
            ...
            expValMap.put(expirationValueField, Bytes.toBytes(hashedKey));
            Map<String, Map<String, byte[]>> expCfMap = new HashMap<String, Map<String, byte[]>>();
            expCfMap.put(expirationColumnFamily, expValMap);
   ...
   public boolean remove(Object key) throws CacheLoaderException {
      log.debug("In HBaseCacheStore.remove for key " + key);
   ...
   public InternalCacheEntry load(Object key) throws CacheLoaderException {
      log.debug("In HBaseCacheStore.load for key " + key);

      String hashedKey = hashKey(this.entryKeyPrefix, key);
      List<String> colFamilies = new ArrayList<String>();
      colFamilies.add(entryColumnFamily);

      try {
         Map<String, Map<String, byte[]>> resultMap = hbf.readRow(this.entryTable, hashedKey,
                  colFamilies);
         if (resultMap.isEmpty()) {
            log.debug("Key " + hashedKey + " not found.");
            return null;
         }
   ...
   public Set<InternalCacheEntry> load(int numEntries) throws CacheLoaderException {
      log.debug("In HBaseCacheStore.load for " + numEntries + " entries.");
   ...
   private Set<Object> loadAllKeysForTable(String table, Set<Object> keysToExclude)
            throws CacheLoaderException {
      log.debug("In HBaseCacheStore.loadAllKeys for " + table);
