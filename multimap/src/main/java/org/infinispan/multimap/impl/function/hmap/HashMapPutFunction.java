...
public class HashMapPutFunction<K, HK, HV> extends HashMapBucketBaseFunction<K, HK, HV, Integer> {
   ...
   @ProtoFactory
   HashMapPutFunction(List<KeyValuePair<HK, HV>> entries, boolean putIfAbsent) {
      this.entries = entries.stream().collect(Collectors.toMap(KeyValuePair::getKey, KeyValuePair::getValue)).entrySet();
      this.putIfAbsent = putIfAbsent;
   }

   @ProtoField(value = 1, collectionImplementation = ArrayList.class)
   List<KeyValuePair<HK, HV>> getEntries() {
      return entries.stream().map(e -> new KeyValuePair<>(e.getKey(), e.getValue())).collect(Collectors.toList());
   }
