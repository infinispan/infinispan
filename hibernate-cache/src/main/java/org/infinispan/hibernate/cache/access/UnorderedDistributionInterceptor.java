            ...
            if (ctx.isOriginLocal() && command.isSuccessful()) {
               ...
               // unlocking (and committing) the entry.
					if (isSynchronous(command)) {
						return rpcManager.invokeCommand(owners, command, new MapResponseCollector(true, owners.size()),
			...
			if (isSynchronous(writeCmd)) {
				if (finalOwners != null) {
					return rpcManager.invokeCommand(
							finalOwners, writeCmd, new MapResponseCollector(true, finalOwners.size()), rpcManager.getSyncRpcOptions());
				} else {
					return rpcManager.invokeCommandOnAll(writeCmd, new MapResponseCollector(true), rpcManager.getSyncRpcOptions());
				}
			} else {
