...
public class Parser52 implements ConfigurationParser {
   ...
   private void parseTakeOffline(XMLExtendedStreamReader reader, BackupConfigurationBuilder backup) throws XMLStreamException {
      ...
      while (reader.hasNext() && (reader.nextTag() != XMLStreamConstants.END_ELEMENT)) {
         count++;
         Element52 takeOffline = Element52.forName(reader.getLocalName());
         for (int i = 0; i < reader.getAttributeCount(); i++) {
            ParseUtils.requireNoNamespaceAttribute(reader, i);
            String value = replaceProperties(reader.getAttributeValue(i));
            Attribute attribute = Attribute.forName(reader.getAttributeLocalName(i));
   ...
   private void setPreloadShared(StoreConfigurationBuilder cclb, int index) {
      if (preload != null && (index == 0)) {
         cclb.preload(preload);
      }
      if (shared != null)
         cclb.shared(shared);
   }
      ...
      for (int i = 0; i < reader.getAttributeCount(); i++) {
         ...
         switch (attribute) {
            case LOCATION:
               ...
               break;
            case FSYNC_INTERVAL:
               log.trace(Attribute.FSYNC_INTERVAL + " is deprecated and ignored in this version of the product.");
               break;
            case FSYNC_MODE:
               log.trace(Attribute.FSYNC_MODE + " is deprecated and ignored in this version of the product.");
               break;
   ...
   public static void parseCommonStoreAttributes(XMLExtendedStreamReader reader, int i,
      ...
      switch (attribute) {
         ...
         case PURGE_SYNCHRONOUSLY:
            log.trace(Attribute.PURGE_SYNCHRONOUSLY + " is deprecated and ignored in this version of the product.");
            break;
         case PURGER_THREADS:
            log.trace(Attribute.PURGER_THREADS + " is deprecated and ignored in this version of the product.");
            break;
         default:
            throw ParseUtils.unexpectedAttribute(reader, i);
      }
   ...
   private void parseStoreChildren(final XMLExtendedStreamReader reader, final StoreConfigurationBuilder<?, ?> storeBuilder, PersistenceConfigurationBuilder persistence) throws XMLStreamException {
      while (reader.hasNext() && (reader.nextTag() != XMLStreamConstants.END_ELEMENT)) {
      ...
      for (int i = 0; i < reader.getAttributeCount(); i++) {
         ...
         switch (attribute) {
            case MARSHALLER_CLASS: {
               builder.serialization().marshaller(Util.<org.infinispan.commons.marshall.Marshaller>getInstance(value, holder.getClassLoader()));
               break;
            }
            case VERSION: {
      ...
      while (reader.hasNext() && (reader.nextTag() != XMLStreamConstants.END_ELEMENT)) {
         ...
         switch (element) {
            case ADVANCED_EXTERNALIZER: {
               ...
               org.infinispan.commons.marshall.AdvancedExternalizer<?> advancedExternalizer = null;
               Integer id = null;
