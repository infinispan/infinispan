...
public class CacheInfoOld {
   ...
   public CacheInfoOld(WrappedBytes cacheName, FailoverRequestBalancingStrategy balancer, int topologyAge, List<InetSocketAddress> servers, ClientIntelligence intelligence) {
   ...
   public CacheInfoOld withNewServers(int topologyAge, int topologyId, List<InetSocketAddress> servers) {
      return withNewServers(topologyAge, topologyId, servers, clientTopologyRef.get().getClientIntelligence());
   }

   public CacheInfoOld withNewServers(int topologyAge, int topologyId, List<InetSocketAddress> servers, ClientIntelligence intelligence) {
      return new CacheInfoOld(cacheName, balancer, topologyAge,  servers, null, -1, clientTopologyRef, new ClientTopology(topologyId, intelligence));
   }

   public CacheInfoOld withNewHash(int topologyAge, int topologyId, List<InetSocketAddress> servers,
                                   ConsistentHash consistentHash, int numSegments) {
      return new CacheInfoOld(cacheName, balancer, topologyAge, servers, consistentHash, numSegments, clientTopologyRef, new ClientTopology(topologyId, getIntelligence()));
   }

   private CacheInfoOld(String cacheName, FailoverRequestBalancingStrategy balancer, int topologyAge,
