...
public class CacheOperationsFactory implements HotRodConstants {
   ...
   public <K, V> ReplaceIfUnmodifiedOperation<K, V> newReplaceIfUnmodifiedOperation(K key, byte[] keyBytes, byte[] value, CacheWriteOptions options, long version, DataFormat dataFormat) {
      return new ReplaceIfUnmodifiedOperation<>(cacheContext, key, keyBytes, value, options, version, dataFormat);
   }
   ...
   public <K, V> ReplaceOperation<K, V> newReplaceOperation(K key, byte[] keyBytes, byte[] values, CacheWriteOptions options, DataFormat dataFormat) {
      return new ReplaceOperation<>(cacheContext, key, keyBytes, values, options, dataFormat);
   }
