...
You can specify a bookmark name instead of a URI. If the argument does not
contain `://`, the CLI treats it as a bookmark name and resolves the URI and
credentials from the stored bookmark.
...
SYNOPSIS
--------
*benchmark* ['OPTIONS'] ['URI' | 'BOOKMARK_NAME']
...
`benchmark prod` +
Performs a benchmark test using the `prod` bookmark. The URI and credentials
are loaded from the stored bookmark.
