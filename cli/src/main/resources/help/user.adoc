...
*--random*::
Whether the username and/or password should be randomly generated.

*--random-character-class*::
The character class to use when generating random identifiers. One of:
* `[:alpha:]` all letters
* `[:alnum:]` all letters and digits
* `[:digit:]` all digits
* `[:graph:]` all printable characters, not including spaces
* `[:lower:]` al lower case letters
