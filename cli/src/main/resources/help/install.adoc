...
* Maven coordinates using the `groupId:artifactId:version` format, for example `org.postgresql:postgresql:42.3.1`.
  For `org.infinispan` artifacts, the version can be omitted and will be inferred from the target server installation.
...
*-d, --with-dependencies*::
Also installs dependencies declared by the artifact.
Some artifacts include a `META-INF/infinispan-dependencies.txt` file listing their required external dependencies.
When this option is set, those dependencies are automatically downloaded and installed.
Dependencies already present in the `server/lib` directory are skipped.
...
`install -d org.infinispan:infinispan-xml-transcoder` +
Installs the XML transcoder module and its dependencies, using the server's version automatically.
