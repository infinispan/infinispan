...
[NOTE]
====
If you need to enable persistence for an existing cache on {k8s}, you must delete the cache and then re-create with persistent enabled. 
Deleting the cache removes all data for the cache, so consider copying any important cache data before you delete the cache.
====
...
include::{topics}/con_soft_index_file_stores.adoc[leveloffset=+2]
include::{topics}/proc_configuring_file_stores.adoc[leveloffset=+2]
include::{topics}/con_single_file_cache_stores.adoc[leveloffset=+2]
