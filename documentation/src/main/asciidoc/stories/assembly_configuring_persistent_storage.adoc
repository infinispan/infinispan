...
Configuring {brandname} as a caching layer in front of persistent storage simplifies data access for applications because {brandname} handles all
interactions with the external storage.

Data overflow::
Using eviction and passivation techniques ensures that {brandname} keeps only frequently used data in-memory and writes older entries to persistent storage.
...
include::../topics/proc_configuring_caches_jndi_names.adoc[leveloffset=+2]
include::../topics/ref_server_connection_pool_properties.adoc[leveloffset=+2]
include::../topics/proc_configuring_jdbc_agroal_properties.adoc[leveloffset=+1]
