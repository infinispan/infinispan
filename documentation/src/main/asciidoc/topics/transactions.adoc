...
Utilizing a pessimistic transaction could lead to deadlocks when multiple nodes try to acquire an overlapping set of keys simultaneously.
By default, the transactions will hang until the lock acquisition timeout elapses, thus releasing the locks and rolling back the transactions.

{brandname} provides a mechanism to detect and resolve deadlocks running alongside the transactions to prevent such scenarios.
To enable deadlock detection, update the transaction XML element by adding the `deadlock-detection` property:

.XML
[source,xml,options="nowrap",subs=attributes+,role="primary"]
----
<transaction deadlock-detection="true" />
----
...
The algorithm is an edge-chasing algorithm that identifies deadlock cycles on demand.
It runs along the transactions and aborts transactions in a deadlock cycle.
The algorithm aims to roll back the smallest number of transactions possible.
The algorithm does not identify deadlock cycles during network partition if the cycle includes nodes in different partitions.
However, cycles within a single partition are detected and resolved.

NOTE: Deadlock detection only enables with pessimistic transactions.
...
=== Forcing write locks on keys in pessimistic transactions
