...
* {jboss-tm-lookup}:
If you are running {brandname} in a standalone environment, or in JBoss AS 7 and earlier, and WildFly 8, 9, and 10, this should be your default choice for transaction manager.
It is a fully fledged transaction manager based on link:http://narayana.io/[JBoss Transactions] which overcomes all the deficiencies of the `EmbeddedTransactionManager`.

* {wildfly-tm-lookup}:
If you are running {brandname} in WildFly 11 or later, this should be your default choice for transaction manager.
...
* `auto-commit` - if enabled, the user does not need to start a transaction manually for a single operation. The transaction is automatically started and committed.
 Default is `true`.
...
More details about the configuration settings are available in link:{configdocroot}index.html[Configuration reference].
...
The following is a more detailed example that should help understand the difference between `READ_COMMITTED` and `REPEATABLE_READ` in the context of {brandname}.
...
So, if you are going to retrieve the same key multiple times within a transaction, you should use `REPEATABLE_READ`.
...
.. If it succeeds, it sends back a positive reply;
...
. The prepare operation is sent to all the owners.
...
Pessimistic transactions are more costly by their nature: each write operation potentially involves an RPC for lock acquisition.
...
There are situations where {brandname} is not required to be a participant in the transaction, but only to be notified by its lifecycle (prepare, complete), for example when {brandname} is used as a second level cache in Hibernate.

{brandname} allows transaction enlistment through {sync}.
To enable it, use `NON_XA` transaction mode.
...
For example, Hibernate second level cache: if {brandname} registers itself with the `TransactionManager` as a `XAResource` than at commit time, the `TransactionManager` sees two `XAResource` (cache and database) and does not make this optimization.
...
For example, consider the "Hello world!" of transactions: transferring money from one bank account to the other.
...
=== API
Once you have configured your cache to use batching, you use it by calling `startBatch()` and `endBatch()` on `Cache`. For example:
...
For this it uses a very simple (for example, no recovery) internal `TransactionManager` implementation.
...
If disabled, the `TransactionManager` is not able to work with {brandname} to determine the in-doubt transactions.
...
There might be multiple elements in the status field, for example "PREPARED" and "COMMITTED" in the case the transaction committed on certain nodes but not on all of them.
...
This step is needed because the XID, a byte array, cannot conveniently be passed to the JMX tool (for example, JConsole) and then re-assembled on {brandname}'s side.
...
These can be useful, for example, if one wants to set up an automatic completion job for certain in-doubt transactions.
