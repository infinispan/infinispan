...
Most of these methods do not make sense on the remote cache (for example, listener management operations), or correspond to methods that are not supported by local cache as well (for example, containsValue).
