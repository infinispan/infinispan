...
Infinispan does not allow deserialization of custom payloads into Java classes for security reasons. This applies to JSON, XML,
and marshalled byte[] content. To allow certain classes to be deserialized, it's possible to enable on per-class or regex basis,
either via system properties or in the cache manager configuration
