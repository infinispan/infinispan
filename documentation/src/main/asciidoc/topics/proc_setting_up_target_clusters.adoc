...
[IMPORTANT]
====
Hot Rod migration from {brandname} {version_10.x} to {version_16.x} is not supported.
Hot Rod migration from {brandname} {version_11.x} and later to {version_16.x} is supported.

Ensure the network properties for the target cluster do not overlap with those for the source cluster.
