...
+
* CLI: Use the [command]`migrate cluster connect` command.
+
[source,bash,options="nowrap",subs=attributes+]
----
[//containers/default]> migrate cluster connect -c myCache -f remote-store.json
----
+
* REST API: Use the `?action=connect-source` parameter with a POST request that includes the remote store configuration in the payload.
+
