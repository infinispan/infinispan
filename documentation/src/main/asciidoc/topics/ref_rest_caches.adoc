...
* `lifespan` Lifespan, in milliseconds, or `Forever` if entries never expire. Returns a value of `-1` unless you set metadata="true".
* `maxIdle` Maximum idle time, in milliseconds, or `Forever` if entries never expire. Returns a value of `-1` unless you set metadata="true".
* `created` Time the entry was created. Returns a value of `-1` unless you set metadata="true".
* `lastUsed` Last time an operation was performed on the entry. Returns a value of `-1` unless you set metadata="true".
* `expireTime` Time when the entry expires or `Never` for immortal entries. Returns a value of `-1` unless you set metadata="true".
