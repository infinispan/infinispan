...
= Checking entry distribution
Invoking this endpoint returns the nodes the entry is present, identifying the primary owner and backup readers.
...
The root element contains the properties:

* `contains_key` identifies if the key exists in the cache.
* `owners` is a list of nodes that have the key.

Each element in the `owners` list represents a node. The properties are:

* `node_name` is the node name.
* `primary` identifies if the node is the primary owner.
* `node_addresses` is a list with all the node's physical addresses.
