...
For caches with text encodings (for example, text/plain, xml, json), the server returns keys and values as plain text.
...
* `created` Time the entry was created or or `-1` for immortal entries. This field is not returned unless you set metadata="true".
