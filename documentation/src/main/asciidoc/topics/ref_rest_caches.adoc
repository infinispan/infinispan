...
|`flags`
|OPTIONAL
|The flags used to add the entry. See link:{javadocroot}/org/infinispan/context/Flag.html[Flag] for more information.
...
For caches with text encodings (for example, `text/plain`, `application/xml`, `application/json`), the server returns keys and values as plain text.
For caches with binary encodings, the server will return the entries as JSON if the conversion is supported, otherwise in a text hexadecimal format, for example `0xA123CF98`.
...
* `timeToLiveSeconds` Based on the entry lifespan but in seconds, or `-1` if the entry never expires. This field is not returned unless you set metadata="true".
* `maxIdleTimeSeconds` Maximum idle time, in seconds, or `-1` if entry never expires. This field is not returned unless you set metadata="true".
* `created` Time the entry was created or `-1` for immortal entries. This field is not returned unless you set metadata="true".
* `lastUsed` Last time an operation was performed on the entry or `-1` for immortal entries. This field is not returned unless you set metadata="true".
* `expireTime` Time when the entry expires or `-1` for immortal entries. This field is not returned unless you set metadata="true".
