...
* `hit_count` displays the total number of results from the query.
* `hit_count_exact` is `true` whether the `hit_count` is exact, otherwise it is a lower-bound.
...
|`hit_count_accuracy`
|OPTIONAL
|Limit the required accuracy of the hit count for the indexed queries to an upper-bound. The default is `10_000`. You can change the default limit by setting the `query.hit-count-accuracy` cache property.
