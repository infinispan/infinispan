...
* `hit_count_exact` is `true` which means the `hit_count` is exact. When the value is `false`, it implies that the hit count value is a lower bound.
...
[id='rest_v2_query_update']
== Update by query
Update multiple entries using update by query in remote caches with `PUT` requests and the `?action=updateByQuery&query` parameter from any HTTP client.

[source,options="nowrap",subs=attributes+]
----
PUT /rest/v2/caches/{cacheName}?action=updateByQuery&query={ickle query}
----
...
[source,options="nowrap",subs=attributes+]
----
POST /rest/v2/caches/{cacheName}?action=updateByQuery
----

.Update statement in request body
[source,json,options="nowrap",subs=attributes+]
----
include::json/rest_query_update_request_body.json[]
----
...
* `non_indexed` Provides details about queries that did not use the index.
...
* `indexed_class_names` Provides the class names of the indexes present in the cache. For Protobuf the value is always `org.infinispan.server.core.query.impl.indexing.ProtobufValueWrapper`.
