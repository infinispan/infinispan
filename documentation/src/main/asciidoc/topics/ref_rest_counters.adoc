...
= Resetting Counters
Restore the intial value of counters without `GET` requests and the
...
Increment counter values with `GET` request` and the `?action=increment`
parameter.
...
Add arbitrary values to counters with `GET` requests that include the
...
Decrement counter values with `GET` requests and the `?action=decrement`
parameter.
