...
You can configure Java Virtual Machine (JVM) settings for {brandname} by editing the `server.conf` configuration file or by setting environment variables.
{brandname} organizes JVM options into separate categories that you can override independently.
