...
Query<Transaction> q = remoteCache.query("from sample_domain.Transaction where amount > 20");
...
[NOTE]
====
If you do not explicitly set the `maxResults` for a query instance, {brandname} limits the number of results returned by the query to `100`.
...
* `Query.executeStatement()` runs a DELETE or UPDATE statement and modifies data.
