...
By default, for performance reasons the accuracy of the hit count is limited to `10,000`.
This means that if the effective hit count is higher than `10,000`, the value couldn't be returned.
In this case a `null` (if the HotRod client or embedded query API is used) or `-1L` (if the REST query API is used) will be returned for it.
To get a proper result this value could be raised up to `Integer.MAX`, but this could badly affect the query performances.
...
Sizing wisely the hit count accuracy can huge improve the query time,
in particular if the query is executed on large indexes, or we don't require an accurate hit count.
