...
This means that, if the effective hit count is higher than `10,000`, the value cannot be returned.
In this case a `null` (if the HotRod client or embedded query API is used) or `-1L` (if the REST query API is used) will be returned.
