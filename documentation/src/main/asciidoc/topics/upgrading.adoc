...
As an example, when `names-as-tags` is set to `false`,
we got 2 different metrics with the channel name distinguishing between them.
...
When `names-as-tags` is set to `true`, we got a single metric:
...
The following defaults have changed for the Hot Rod client:

* `connection_timeout` is now 2000 ms / 2 seconds (previously 60000 ms / 60 seconds)
...
The scattered cache mode have been removed a bit earlier. Please use Distributed caches instead.
...
Due to incosistency issues with concurrent updates the NearCache SPI needed to be updated to have concurrent
...
The following table summarizes the mapping of fields between Hibernate Search 5 annotations and {brandname} native annotations:

[%header,cols=3*]
|===
| HS5 annotations | {brandname} native annotations| Description

| @Field(index=Index.YES) | searchable | Fields previously marked as indexed are now searchable.

| @Field(store = Store.YES) | projectable = true | Fields previously marked as stored are now projectable.

| type String && @Field(analyze = Analyze.YES) | analyzer = "<definition>"| String fields that were marked with analyzer definitions remain being analyzed during indexing.

| @Field(analyze = Analyze.NO) && (@Field(store = Store.YES) OR @Field(sortable = Sortable.YES)) | sortable = true| Fields that were not analyzed but were either stored in the index or marked as sortable are now considered sortable.

| N/A | aggregable = true| Performing aggregation operations using the Hibernate 5 Search annotations was not possible.

| N/A | normalizer = "lowercase"| Mapping fields that were analyzed or normalized is not possible due to the potential data loss in the process.
|===
