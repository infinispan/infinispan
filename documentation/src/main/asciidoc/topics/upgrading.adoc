...
== Embedded Programmatic Configuration
The various builders used for programmatically configuring an embedded cache have been updated so that it's no longer
necessary to call `enable()` or `enable(true)` on a configuration element if one or more if it's attributes have been
...
new GlobalConfigurationBuilder().globalState()
   .enable()
   .persistentLocation("some-location")
   .build();
----
