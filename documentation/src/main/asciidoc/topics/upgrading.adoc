...
== JCache managed interceptor names
The managed JCache interceptors, which allow using JCache annotations like `@CacheResult` with
`EmbeddedCacheManager` instances provided by the application server, have moved from package
`org.infinispan.jcache.annotation` to `org.infinispan.jcache.embedded.annotation`.
