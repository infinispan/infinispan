...
In order to turn on Cloud support for {brandname} library mode, one needs to add a specific dependency according to the cloud provider in use.
...
.NATIVE_S3_PING support in library mode
[source,xml,options="nowrap",subs=attributes+]
----
include::dependencies_maven/ispn_cloud_s3.xml[]
----
...
.AZURE_PING support in library mode
[source,xml,options="nowrap",subs=attributes+]
----
...
=== GOOGLE2_PING
GOOGLE2_PING uses Google Cloud Storage (GCS) to store information about the cluster members.
...
.GOOGLE2_PING support in library mode
[source,xml,options="nowrap",subs=attributes+]
----
...
[source,yaml]
----
include::config_examples/rolling_updates.yaml[]
----
