...
= Configuring TLS versions, cipher suites and named groups

When using SSL/TLS encryption to secure your deployment, you can configure {brandname} Server to use specific versions of the TLS protocol as well as specific cipher suites and allowed key exchange schemes via named groups within the protocol.
...
. Configure {brandname} to use a specific set of named groups for hybrid key exchange instead of relying on the JDK defaults with the `enabled-named-groups` attribute.
