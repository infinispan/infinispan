...
{brandname} servers can require authentication for client connections with different mechanisms.

==== SCRAM
Provides username and password authentication with the (_"default"_) realm on {brandname} servers.
...
==== DIGEST
Deprecated by SCRAM and available only for backwards compatibility with
previous versions of {brandname} server that do not support SCRAM.
...
==== PLAIN
Sends credentials in clear-text to authenticate with {brandname} servers.
...
==== OAUTHBEARER
Requires client applications to retrieve authentication tokens from OAuth2
providers.
...
[TIP]
====
You should configure clients with a `TokenCallbackHandler` to refresh OAuth2
...
==== EXTERNAL
Uses certificates to authenticate with {brandname} servers. You provide clients
with truststores to validate server certificates and keystores to supply client
certificates.
...
link:#hr_encryption[Setting Up Encryption]

==== GSSAPI and Kerberos
Uses the Kerberos protocol to provide clients with authentication tokens for
centralized security services.

To use the GSSAPI mechanism, you must create a _LoginContext_ for clients to
obtain a Ticket Granting Ticket (TGT) as follows:
...
====
+
. Configure your Hot Rod client for GSSAPI.

.GSSAPI Hot Rod client configuration
...
[source,java]
----
include::code_examples/HotRodSNI.java[]
----
...
[source,java]
----
include::code_examples/HotRodTLSAuth.java[]
----
