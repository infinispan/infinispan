...
tools such as Hibernate OGM.

[[hotrod_java_client]]
== Hot Rod Java clients
Use Java clients to access data through a {brandname} Hot Rod server.
...
include::proc_creating_hotrod_java_client_instances.adoc[leveloffset=+2]
include::proc_configuring_hotrod_java_client_properties.adoc[leveloffset=+2]
...
the client must provide the required information. Aside from credentials, some mechanisms require the security realm name and / or the server name (as configured on the server protocol `authentication` element).

==== SCRAM
The SCRAM family of authentication mechanisms is the recommended approach for simple username/password authentication scenarios.
If you are using the default realm on the server (_"default"_) and `SCRAM-SHA-512` is in the declared server mechanisms,
all you need to do is provide your credentials as follows:

.Hot Rod client configuration with SCRAM-SHA-512 authentication
[source,java]
----
...
==== DIGEST
The DIGEST family of authentication mechanisms has been deprecated in favor of the SCRAM mechs, so you should use those if possible.
However, when connecting to older versions of {brandname} Server, it might be necessary to use it.
Since the DIGEST mechanisms require the realm name, make sure you specify it:
...
.Hot Rod client configuration with PLAIN authentication
[source,java]
----
ConfigurationBuilder clientBuilder = new ConfigurationBuilder();
clientBuilder
...
==== OAUTHBEARER
The OAUTHBEARER mechanism requires interaction with an OAuth2 token provider. It is up to your application to retrieve an OAuth2 token.

.Hot Rod client configuration with OAUTHBEARER authentication
[source,java]
----
...
Since OAuth2 tokens may need to be refreshed, it is better to

.Hot Rod client configuration with OAUTHBEARER callback
[source,java]
----
String token = "..."; // Obtain the token from your OAuth2 provider
...
image::{images_dir}/cert_chain.png[align="center", title="Certificate chain"]
...
reached, near-cached entries are evicted. If providing `0` or a negative value,
it is assumed that the near cache is unbounded.
...
=== Client Intelligence
Client intelligence refers to mechanisms the `HotRod` protocol provides
for clients to locate and send requests to {brandname} servers.
...
The marshaller can be plugged through the "marshaller" configuration element (see Configuration section): the value should be the fully qualified name of a class implementing the link:{javadocroot}/org/infinispan/marshall/Marshaller.html[Marshaller] interface.
This is a optional parameter, if not specified it defaults to the <<protostream,ProtoStreamMarshaller>>
...
==== ProtoStream
Since version 10.0, the default marshaller for the client is the Protobuf based <<protostream,ProtoStreamMarshaller>>.
Keeping your objects stored in protobuf format has the benefit of being able to consume them with compatible clients
written in different languages. To aid with the configuration of the marshaller's SerializationContext, we have extended
the client configuration to allow for SerializationContextInitializer(s) to be provided by the user and applied on
RemoteCacheManager startup.
...
The Hot Rod client can be monitored and managed via JMX.
...
image::{images_dir}/server_modules_6.png[align="center", title="Concurrent readers"]
...
image::{images_dir}/server_modules_7.png[Concurrent updates]
...
image::{images_dir}/server_modules_8.png[align="center", title="P2P solution"]
...
image::{images_dir}/server_modules_9.png[align="center", title="Get versioned"]
...
image::{images_dir}/server_modules_10.png[align="center", title="Replace if versions match"]
