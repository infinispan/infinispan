...
for the duration of the operation. The following is an example on how to perform the
...
be associated with the key, it is recommended to use `find()` to verify
whether there is a value associated with the key.
...
which should be used only when the caller has guarantees that there is
...
lifespan, and last accessed time.
...
link:{javadocroot}/org/infinispan/functional/FunctionalMap.WriteOnlyMap.html#evalMany(java.util.Map,java.util.function.BiConsumer)[`evalMany`]
API:
...
The final type of operations are read-write operations, and within
this category CAS-like (Compare-And-Swap) operations can be found.
These types of operations require previous value associated with the key
...
APIs fall within this category, and they can be implemented using the
...
value present in the map and if it is equal to the value passed in as
first parameter, the second value is stored, returning a boolean
indicating whether the replace operation was successfully completed. This operation
can be implemented using the read-write map API:
...
as version information, lifespan, and last accessed/used time.
Some of these can be provided by the user, such as version and lifespan, but others are computed internally and can only be queried, such as last accessed/used time.

The functional map API provides a flexible way to store metadata parameters
along with a cache entry. To be able to store a metadata parameter, it must
extend
...
link:{javadocroot}/org/infinispan/functional/Param.PersistenceMode.html[`Param.PersistenceMode`]
...
a persistence store. By passing link:{javadocroot}/org/infinispan/functional/Param.PersistenceMode.html#SKIP[`PersistenceMode.SKIP`] as parameter,
the write operation skips the persistence store and its effects are only
seen in the in-memory contents of the cache. link:{javadocroot}/org/infinispan/functional/Param.PersistenceMode.html#SKIP[`PersistenceMode.SKIP`]
...
Note that there is no need for another link:{javadocroot}/org/infinispan/functional/Param.PersistenceMode.html[`Param.PersistenceMode`] option to skip
...
via the link:{javadocroot}/org/infinispan/functional/FunctionalMap.WriteOnlyMap.html[WriteOnlyMap].

Finally, new link:{javadocroot}/org/infinispan/functional/Param.html[Param] implementations are normally provided by the functional
...
created and cache entry modify/update events because they do not have
...
If listening for two or more event types, it is better to pass in an
implementation of
...
this overload if that is possible. Therefore, you can call
...
offerings, so you can still use
link:{jdkdocroot}/java/util/concurrent/ConcurrentMap.html[`ConcurrentMap`]
or
link:https://github.com/jsr107/jsr107spec/blob/v1.0.0/src/main/java/javax/cache/Cache.java[`JCache`]
standard APIs if that is what suits your use case best.
