...
The files are split into subcategories and briefly described below.

=== Environment

These files contain metrics of the environment the {brandname} server is running on.
In addition to understanding how the application behaves, external factors might cause issues, such as misconfiguration in the network or many processes running in the same host.
...
* `ip-route`: Describes the routing table in the host.
