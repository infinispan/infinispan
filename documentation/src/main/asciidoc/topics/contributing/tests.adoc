...
You can do this using `coverage` profile. https://www.eclemma.org/jacoco/[JaCoCo] is used to measure code coverage. While code coverage recording, besides the `coverage` profile you should also add the additional jacoco agent library name so that it is attached to containers while running integration tests.
...
* _Each test class must have a unique test name_
...
Generally speaking, if you expect something will happen, and you don't have a guarantee when, a good approach is to try that expectation in a loop, several times, with a generous (5-10secs) timeout. For example:
...
* For each cache that is created with `TestCacheManagerFactory.createXyzCacheManager()` the test harness will allocate a unique JMX domain name which can be obtained through `CacheManager.getJmxDomain()`.
...
In a multithreaded environment like {brandname}'s testsuite, grepping for these words can greatly help the debugging process.
