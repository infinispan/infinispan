...
To simply obtain the health status of the cache managers, without the need to go through authentication, do a `GET` request:
...
The response will be in `text/plain` with `HEALTHY`,`HEALTHY_REBALANCING` or `DEGRADED`.
