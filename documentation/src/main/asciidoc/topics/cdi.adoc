...
Adding the `@Remote` cache qualifier with the cache name on each injection point might be error prone.
...
TIP: There is now a separate module for JSR 107 (JCACHE) integration, including API.

//old_link: See link:#jcache_jsr_107[this chapter] for details.
...
[source,xml,options="nowrap",subs=attributes+]
----
include::code_examples/InterceptorsUnmanaged.xml[]
----
...
//old_link: TIP: Check link:#listeners_and_notifications[Listeners and Notifications] for more information about event types.
