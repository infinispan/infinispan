...
In particular, if `numOwners` nodes leave in quick succession, some entries will be lost, which means that a distributed cache tolerates `numOwners - 1` node failures.
...
In this case the backup node forwards the write operation to the primary owner, which then replicates the write operation to the backup.
