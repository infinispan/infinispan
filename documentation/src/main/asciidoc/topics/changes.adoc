...
should either 1) migrate to using Protostream based marshalling (recommended), or 2) replace `AdvancedExternalizer`
with `org.jboss.marshalling.Externalizer` implementations and add the `org.jboss.marshalling.Externalize` annotation to their
classes in order to configure this.

All `AdvancedExternalizer` defined declaratively for an older schema version are ignored.

==== Custom Commands

All commands, user and Infinispan internal, are now marshalled using Protostream. Consequently, the
