...
computing power for executing code on all of them.
...
upon the data in a cache and is used instead as a way to help users execute code
in the cluster.

This manager was built specifically around Java's streaming API, thus all methods take a functional
interface as an argument. Also since these arguments will be sent to other nodes they need to be serializable.  A nice
trick is used to ensure the lambdas are immediately Serializable.  That is by having the arguments implement both
Serializable and the real argument type (i.e. Runnable or Function).  The JRE will pick the most specific class when
...
It is possible to limit on which nodes the command will be run. For example, you may
...
The target node can also be chosen by any means using a `Predicate` that
...
and instead submit it to only one. Each submission will possibly use a different node to
...
one it turns out that stem:[\pi = 4\frac{C_a}{S_a}]. Now, imagine shooting a very large
number of darts into a square; taking the ratio of darts that land inside a
circle over the total number of darts shot approximates the stem:[\frac{C_a}{S_a}] value. Since
stem:[\pi = 4\frac{C_a}{S_a}], this ratio can be used to approximate the value of stem:[\pi]. The
more darts you shoot, the better the approximation. In the example below, 1 billion
darts are shot, but instead of "shooting" them serially, the work is parallelized
across the entire {brandname} cluster. Note that, this
