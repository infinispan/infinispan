...
An external actor (for example, a Kubernetes/OpenShift operator, or a human administrator)
...
The module is still very young and does not yet support many {brandname} features.
...
Non-clustered embedded listeners currently receive notifications on all the nodes, not only the node
...
In theory, it would be possible to send in parallel one unicast RPC for the value and one multicast RPC for the address,
...
NOTE: The number of entries not yet inserted is very hard to estimate.
In the future {brandname} may provide a way to limit the overhead of key location information,
for example, by using a distributed cache.
...
because it asks another node for the anchor owner.
