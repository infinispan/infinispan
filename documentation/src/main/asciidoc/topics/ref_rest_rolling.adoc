...
----
POST /v2/caches/{cacheName}/rolling-upgrade/source-connection
----
...
If the operation successfully completes, the service returns 204 (No Content). If the target cluster
is already connected to the source cluster, it returns status 304 (Not Modified).
...
[source,options="nowrap",subs=attributes+]
----
GET /v2/caches/{cacheName}/rolling-upgrade/source-connection
----
...
[source,options="nowrap",subs=attributes+]
----
HEAD /v2/caches/{cacheName}/rolling-upgrade/source-connection
----
