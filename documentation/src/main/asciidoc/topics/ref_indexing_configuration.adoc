...
Useful when you want to run embedded queries from a server task, in the case the cache is Protobuf encoded.
...
|`thread-pool-size`
|This configuration is ignored since {brandname} 15.0. The indexing engine now uses the {brandname} thread pools.
