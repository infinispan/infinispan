...
* `purge` to purge indexes at cache startup time.
* `reindex` to reindex the entities present in cache at cache startup time.
* `auto` with this configuration Infinispan will try to run the right action to align cache data and indexes:
...
* `none` to not trigger any operation. This is the default.

For instance to set `purge` see the following example.
The other configurations are similar.

=== Purge indexes at start

.Purge indexes at startup time
[source,xml,options="nowrap",subs=attributes+]
----
include::xml/indexing_startup_mode.xml[]
----
