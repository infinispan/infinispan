...
3. The cache memory is automatically limited configuring the `memory` `maxCount` or `maxSize`
...
In the case of (1) and (2) and the embedded cache eviction API is used to manually reduce the memory used by the cache,
the recommended configuration is to set `REINDEX` as startup mode for this very use case.
