...
Boolean negation has the highest precedence among logical operators and applies only to the next simple attribute condition.
...
----
UPDATE FROM <entityName> SET <field> = <value> [, SET <field> = <value>]* [WHERE condition]
----

* Reference only single entities with `<entityName>`. UPDATE queries cannot use joins.
* WHERE conditions are optional.
* Use `SET` to assign a new value to a field.
* Use `ADD` to add values to a collection field.
* Use `REMOVE` to remove values from a collection field.

Values can be:

* A single constant: `SET name = 'John'`
* A list of constants in parentheses: `ADD tags = ('new', 'updated')`
* A list of constants in square brackets: `ADD tags = ['new', 'updated']`
