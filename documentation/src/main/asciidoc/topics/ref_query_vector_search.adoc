...
Notice that in order to use this kind of search the entity, in this example `play.Item`, has to be `@Indexed` and
the field, in this example `myVector`, should be annotated with `@Vector`.

Two kinds of vector field types are supported:
...
For the vector you can use either a placeholder for each term of the vector:
...
you can limit the searching set applying classic predicates (match, full-text-search, range, ...) to the kNN queries,
defining a filtering clause.
...
The other mapping attributes are optional, since {brandname} will have a default for each of them.
...
ifdef::backend-html5[stem:[s = \frac{1}{1+d}]]
ifdef::backend-pdf[`s = 1/(1+d)`]
| This similarity cannot be of `zero magnitude`, for example when a vector is all zeroes: `[0,0,0,... 0,0]`, the cosine is not defined and will result in an error.
|===
