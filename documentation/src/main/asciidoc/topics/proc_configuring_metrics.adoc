...
. Enable or disable the inclusion of element names as tags using the `names-as-tags` attribute or field.
...
[discrete]
=== Names as tags
By default, element identifiers, such as the name of the cache manager and the cache, are embedded into the name of the
metric. Enabling `names-as-tags` places these identifiers into tags, making the metric name independent.

.XML
[source,xml,options="nowrap",subs=attributes+,role="primary"]
----
include::xml/metrics_cache_container_names_as_tags.xml[]
----

.JSON
[source,json,options="nowrap",subs=attributes+,role="secondary"]
----
include::json/metrics_cache_container_names_as_tags.json[]
----

.YAML
[source,yaml,options="nowrap",subs=attributes+,role="secondary"]
----
include::yaml/metrics_cache_container_names_as_tags.yaml[]
----
