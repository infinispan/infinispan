...
|`metadata`
|Optional
|A Boolean value that determines whether {brandname} returns additional metadata about the schema. The default value is `false`.
|===

When `metadata` is `true`, {brandname} returns a JSON document that includes the schema content, validation errors, and the list of caches that index types declared in the schema.
