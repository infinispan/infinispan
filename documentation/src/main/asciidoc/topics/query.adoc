...
{brandname} supports a shared index leveraging the {brandname} Directory Provider, which stores indexes in a separate set of caches, called InfinispanIndexManager.
...
[source,java,tile="Range queries with Ickle"]
----
...
[source,java,tile="Phrase queries with Ickle"]
----
...
Apart from supporting indexing and searching of Java entities to embedded
clients, {brandname} introduced support for remote, language neutral, querying.
...
pass:['___schema_manager'] role so they can write to the `___protobuf_metadata`
cache.

.Procedure

To register a schema with {brandname} server, use the {brandname} CLI:

. Start the {brandname} CLI and connect to your {brandname} cluster.
. Register schemas with the **schema** command.
+
...
----
+
. Use the **get** command to verify schemas.
+
...
Alternatively, if you enable JMX, you can invoke the `registerProtofile()`
operation on the _ProtobufMetadataManager_ MBean.

.Reference

* link:{cli_docs}#protobuf_query[{brandname} CLI: Querying Caches with Protobuf Metadata]
