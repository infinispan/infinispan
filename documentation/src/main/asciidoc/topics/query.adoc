...
{brandname} supports shared indexes leveraging the {brandname} Directory Provider, which stores indexes in a separate set of caches. Two index managers are available
...
The AffinityIndexManager is an *experimental* index manager used for shared indexes that also stores indexes using the {brandname} Directory Provider.
...
[source,xml,options="nowrap",subs=attributes+]
----
include::config_examples/indexing_primary_owner.xml[]
----
...
by registering the required .proto files with the `__protobuf_metadata` cache.
