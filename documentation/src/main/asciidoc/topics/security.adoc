...
There are two levels of configuration: global and per-cache. The global configuration defines the set of roles/permissions mappings while each cache can decide whether to enable authorization checks.
...
By default, all roles defined at the global configuration level apply to caches. It is possible to specify a subset of roles:

.Programmatic explicit cache authorization roles
[source,java]
----
  ConfigurationBuilder config = new ConfigurationBuilder();
  config
...
.Declarative explicit cache authorization roles
[source,xml,options="nowrap",subs=attributes+]
----
include::config_examples/cache_authorization_explicit_roles.xml[]
----

With the above configuration, a user who maps to the `writer` role but not to any of the other roles will be denied access.
