...
If however you are not able to use the `AsyncCache` and `CacheStream` interfaces you cannot utilize `Serializable`
...
the `AsyncCache` that is backing this Stream.  If your `Consumer` implements the
