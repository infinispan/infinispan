...
similar to MapReduce. Streams, like MapReduce, allow you to perform processing
...
Also, since {brandname} can control how the entries are iterated upon, it can more efficiently perform the operations in a distributed cache, allowing all the operations across the cluster to run concurrently.
...
A stream used with a local or invalidation cache can be used the same way you would use a stream on a
regular collection. {brandname} handles all the translations, if necessary, behind the scenes and works with all
the more interesting options (that is, storeAsBinary and a cache loader).  Only data local to
...
new node joins or leaves. Distributed Streams handle this reshuffling of data automatically so you do not
have to worry about monitoring when nodes leave or join the cluster.
Reshuffled entries may be processed a second time, and {brandname} keeps track of the processed entries at the
...
The performance gain for most operations when a rehash does not occur is completely negligible.
...
The simplest way is to use a CacheStream instance and use a lambda as you would normally.
{brandname} overrides all the various Stream intermediate and terminal methods to take
Serializable versions of the arguments (that is, SerializableFunction, SerializablePredicate, and similar)
You can find these methods at
link:{javadocroot}/org/infinispan/CacheStream.html[CacheStream].
...
In the previous example, a `Collector` was used to collect all the results into a `Map`.
Unfortunately, the link:{jdkdocroot}/java/util/stream/Collectors.html[Collectors]
class does not produce Serializable instances.  Thus, if you need to use these, there are two ways to do so:
...
it is on a per-request basis. The former means the local execution will not time out, and the latter means if you have a failover
...
For more information about this, please check the Javadoc in
...
Distributed streams execution works in a fashion very similar to MapReduce.  Except, in this case, zero to many intermediate operations are sent
(map, filter, and others) and a single terminal operation to the various nodes.  The operation comes down to the following:
...
everything as normal, except it does not perform the final
...
here to remember is that the final value does not have to be serializable, but rather
...
method adds the numbers together from each node.

findAny findFirst::
The link:{jdkdocroot}/java/util/stream/Stream.html#findAny--[findAny]
operation returns the first value found, whether it was from a remote node
...
a value coming from the combiner does not have to be Serializable.
...
track of what keys per segment have been processed instead of only segments.  This is needed
...
in a completed batch will be run again when the rehash failure operation occurs.  Note that, adding a
node will not cause this issue as the rehash failover does not occur until all responses are received.
...
of MapReduce paradigm. Assume you have a mapping of key -> sentence stored on
{brandname} nodes. Key is a String, each sentence is also a String, and you need
...
However, what if you want to find the most frequent word in the example?  If you take a second
to think about this case, you will realize you need to have all words counted  and available
locally first. Thus, there are actually a few options.

You could use a finisher on the collector, which is invoked on the user thread
...
Unfortunately, the last step is only going to be run in a single thread, which, if there are a lot of
words, could be quite slow.  Maybe there is another way to parallelize this with Streams.

As mentioned before, the processing is on the local node after collection, so you could actually use
a stream on the map results.  You can therefore use a parallel stream on the results.
...
If you carefully note what is serialized and what is not, you notice that only the word along
...
cache. The cache is not captured by the lambda, as {brandname} provides a special BiConsumer method
...
{brandname} has specifically added a new variant which is called `LockedStream` to address this.

*Plenty of other examples*

The `Streams` API is a JRE tool and there are lots of examples for using it.
Remember that your operations need to be Serializable in some way.
