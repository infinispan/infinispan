...
== Create an embedded cache
Creating an embedded cache, using default configuration options as defined by the
...
== Use a custom embedded configuration

It is possible to use a custom embedded {brandname} configuration file by
passing the URI pointing to it to the `CachingProvider.getCacheManager(URI)` call:
...
== Use a custom remote configuration

It is possible to use a custom remote (Hot Rod) {brandname} configuration file by
passing the URI pointing to it to the `CachingProvider.getCacheManager(URI)` call:
...
[source,xml,options="nowrap",subs=attributes+]
----
include::config_examples/jcache_clustering.xml[]
----
