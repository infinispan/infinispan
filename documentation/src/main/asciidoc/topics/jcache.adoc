...
----
include::dependencies_maven/jcache_embedded.xml[]
----

For remote caches, a different dependency is needed:
.pom.xml
[source,xml,options="nowrap",subs=attributes+]
----
include::dependencies_maven/jcache_remote.xml[]
----
...
(just like {brandname} with OBJECT storage or JDK Collections work).
