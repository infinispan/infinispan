...
[NOTE]
====
Enable `BASIC` intelligence to use the Hot Rod client configuration when a {brandname} Server cluster does not send internal and hidden cluster topology to the Hot Rod client.
====
...
If the Hot Rod client is configured to use multiple {brandname} clusters, it is possible to have
different intelligence for each cluster.
If not specified, it uses the intelligence configured globally.
