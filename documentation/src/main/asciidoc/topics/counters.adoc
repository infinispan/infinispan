...
* link:{configdocroot}infinispan-counters-config-{schemaversion}.html[{brandname} configuration schema reference]
...
If you do not want to use the async API, it is possible to return a synchronous counter via `sync()` method.
...
On the other hand, the reads do not acquire any locks and read the current value.
...
NOTE: An operation is considered completed when the ``CompletableFuture`` is completed.
...
Its main advantage over the `StrongCounter` is the lower contention in the cache.
...
They are similar to the `StrongCounter` methods, but they do not return the new value.
...
Collecting statistics is a good example of such a use case.
...
The `Handle` object returned has the main goal to remove the `CounterListener` when it is no longer needed.
