...
[source,xml,options="nowrap",subs=attributes+]
----
include::config_examples/clustered_counters_config.xml[]
----
...
* `lifespan` [strong]: Sets the strong consistent counter's lifespan in milliseconds.
After this time, the counter is removed and its value reset to the counter's initial value.
A `lifespan` lower or equals to `0` (zero), disables this feature.
Default value is `-1`.
...
WARNING: The `lifespan` is an experimental feature and may be changed or removed in the future.
...
To use a strong counter as rate limiter, configure `upper-bound` and `lifespan` as follows:
...
NOTE: this is an experimental feature and may be removed in a future.
