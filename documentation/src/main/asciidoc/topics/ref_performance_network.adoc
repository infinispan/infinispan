...
To set it to the same size as the send socket above, add the following line to _/etc/sysctl.conf_:
+
----
...
To set it to the same size as the receive socket above, add the following line to _/etc/sysctl.conf_:
+
----
