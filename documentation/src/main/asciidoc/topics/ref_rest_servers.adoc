...
[id='rest_v2_server_caches_defaults']
= Retrieving Default values for Cache Configuration
Retrieve default values for cache configuration with `GET` requests.
...
{brandname} responds with the default values for cache configuration in JSON format.
