...
[id='rest_v2_cache_manager_stats_reset']
= Resetting Cache Manager Statistics
Reset the statistics for Cache Managers with `POST` requests.

[source,options="nowrap",subs=attributes+]
----
POST /rest/v2/cache-managers/{cacheManagerName}/stats?action=reset
----
