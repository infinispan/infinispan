...
* `create-cache`, `update-cache`, `create-template`, `remove-template`: the declarative configuration of the entity that has been created in the format negotiated by the `Accept` header.
[source,options="nowrap",subs=attributes+]
----
event: create-cache
data: infinispan:
