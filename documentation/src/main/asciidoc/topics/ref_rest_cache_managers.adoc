...
data: "}}
----
* `remove-cache`, `remove-template`, `remove-schema`: the name of the removed entity.
[source,options="nowrap",subs=attributes+]
----
event: remove-cache
data: myCache
...
This endpoint returns only the `cache`, `template` and `schema` events which are also returned by <<rest_v2_container_listen>>.
