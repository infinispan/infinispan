...
For possible values, check the link:{javadocroot}/org/infinispan/lifecycle/ComponentStatus.html[`org.infinispan.lifecycle.ComponentStatus`] documentation
...
* `rebalancing_enabled` is true if rebalancing is enabled. Fetching this property might fail on the server. In that case the property is not present in the payload.
...
be manually terminated shortly after this endpoint is invoked. Once this method has been called, it is not possible to
...
The `event` value will be one of:

* `create-cache`, `remove-cache`, `update-cache`: a cache was created, removed or its configuration updated
* `create-template`, `remove-template`, `update-template`: a template was created, removed or updated
* `create-schema`, `remove-schema`, `update-schema`: a schema was created, removed or updated
* `create-counter`, `remove-counter`: a counter was created or removed
* `lifecycle-event`: container lifecycle events, such as nodes joining and leaving the cluster

The `data` value will contain information about the event:

* `create-cache`, `update-cache`, `create-template`, `update-template`: the declarative configuration of the entity that has been created in the format negotiated by the `Accept` header. The following example uses `application/yaml`:
[source,options="nowrap",subs=attributes+]
----
event: create-cache
data: infinispan:
data:   cacheContainer:
data:     myCache:
data:       replicatedCache:
data:         mode: "SYNC"
----
* `create-schema`, `update-schema`: a response structured in the negotiated media type. The following example uses `application/json`:
[source,options="nowrap",subs=attributes+]
----
event: create-schema
data: {"schema":{"name":"address.proto","errors":"","proto":"message Address {
data:     string street = 1;
data: }
data: "}}
----
* `create-counter`: the counter configuration in the negotiated media type. The following example uses `application/json`:
[source,options="nowrap",subs=attributes+]
----
event: create-counter
data: {"strong-counter":{"name":"my-counter","initial-value":0,"storage":"VOLATILE","upper-bound":100}}
----
* `remove-cache`, `remove-template`, `remove-schema`, `remove-counter`: the name of the removed entity.
[source,options="nowrap",subs=attributes+]
----
event: remove-cache
data: myCache
----
* `lifecycle-event`: the event payload
[source,options="nowrap",subs=attributes+]
----
...
data:     detail: "A more detailed message"
data:   meta:
data:     instant: "2025-11-21T11:25:33.554122447Z"
data:     context: "myCache"
data:     scope: ~
data:     who: ~
----
...
This endpoint returns only the `cache`, `template`, `schema` and `counter` events which are also returned by <<rest_v2_container_listen>>.
