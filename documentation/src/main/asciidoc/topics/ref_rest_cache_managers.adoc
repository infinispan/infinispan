...
[WARNING]
====
This endpoint is deprecated in favor of <<rest_v2_container_listen>>.
Use `GET /rest/v2/container?action=listen&category=config` instead.
====
...
The emitted events come from logged information, so each event contains an identifier associated with the message, or from configuration changes.
The parameter `category` allows to filter for the interested events.

The `event` value depends on the category selected (see Request Parameters).
The table below summarizes the available categories, and the events it triggers:

|===
|Category |Event

m|config
|`create-cache`, `remove-cache`, `update-cache`, `create-template`, `remove-template` or `update-template`
...
The `data` has the logged information, which includes the `message`,
...
|`includeCurrentState`
|OPTIONAL
|If `true`, the results include the state of the existing configuration in addition to the changes.
If set to `false`, the request returns only the changes.
The default value is `false`.

|`pretty`
|OPTIONAL
|If `true` returns formatted content, including additional spacing and line separators which improve readability but increase payload size.
The default is `false`.

m|category
|OPTIONAL
|The event log category to stream.
If not specified, it will send events from the categories `config` and `lifecycle`.
