...
. Annotate your Java objects with `@ProtoField` and `@ProtoFactory`.
+
[source,java]
.Author.java
----
...
----
+
. If necessary, create an adaptor class for any external, third-party Java object classes.
. Annotate the adaptor class with `@ProtoAdaptor`.
+
[source,java]
----
