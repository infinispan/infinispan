...
Annotate Java objects in your classes to include them in an implementation of the ProtoStream `SerializationContextInitializer` interface so that the ProtoStream library can encode your Java objects to Protobuf format and {brandname} can transmit and store them.

.Prerequisites

{brandname} provides a `protostream-processor` artifact that processes Java annotations in your classes at compile time.
The processor generates Protobuf schemas, marshallers, and a concrete implementation of the `SerializationContextInitializer` interface that you can use to initialize a ProtoStream `SerializationContext`.
...
{brandname} is ready to marshall your Java objects.
