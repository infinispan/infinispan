[id='hotrod_failover']
= Client Failover
Hot Rod clients can automatically fail over when {brandname} cluster topologies
...
can fail over between {brandname} clusters.
...
Even though Hot Rod clients can fail over, you should use transactional caches
