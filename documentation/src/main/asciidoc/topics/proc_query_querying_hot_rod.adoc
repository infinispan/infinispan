...
----
+
. Implement the `SerializationContextInitializer` interface in a new class and then add the `@ProtoSchema` annotation.
...
* link:{encoding_docs}[Encoding Infinispan caches and marshalling data]
* link:{encoding_docs}#protostream-annotations_marshalling[Encoding {brandname} caches and marshalling data: ProtoStream annotations]
