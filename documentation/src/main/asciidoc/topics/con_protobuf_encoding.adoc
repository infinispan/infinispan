...
As a cache encoding, Protobuf gives you excellent performance as well as interoperability between client applications in different programming languages and remote server endpoints.
...
Because it is language neutral, Protobuf encoding means {brandname} can handle requests from client applications written in Java, C{plusplus}, C#, Dart, Python, Go, and more.

//Downstream content
ifdef::downstream[]
[TIP]
====
Always use Protobuf encoding for integration with any Red Hat technology because it allows communication between applications and services.
====
endif::downstream[]

Protobuf also enables clients on different remote endpoints, Hot Rod or REST, to operate on the same data.
Because it is backed by the REST API, you can access and work with Protobuf-encoded caches through {brandname} Console.

[NOTE]
====
{brandname} uses a ProtoStream library to encode caches with the `application/x-protostream` media type.
This encoding is similar to `application/x-protobuf` but the two media types have some differences that mean they are not interchangeable.
