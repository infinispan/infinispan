...
different annotations and event classes are used. The following is an example of a
...
When a client event listener is not needed any more, it can be removed:
...
The events generated by default contain enough information to make the
...
If you want to do both event filtering and customization, it is easier to
...
For convenience, it is recommended to extend
...
converter factory names, for example:
...
it is not necessary to deploy a custom marshaller since the format is already supported by the server: there are
transcoders from Protobuf format to most common formats like JSON and POJO.

When using filters/converters with those caches, and it is desirable to use filter/converters with Java Objects rather than
binary Protobuf data, it is necessary to configure the extra ProtoStream marshallers so that the server can unmarshall
...
added or when there is a failover of the listener.
...
called `includeCurrentState` which is set to true, when the failover happens,
the cache contents can be iterated over and `ClientCacheEntryCreated` events
