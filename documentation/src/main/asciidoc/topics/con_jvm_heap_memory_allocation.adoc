...
| `x` | Represents the average size of a key-value pair. You can determine the value of `x` by using the following calculator: `x =  Serialized size of key + Serialized size of value + Metadata`
...
| `p` | 
Denotes a fractional value for the full heap space that {brandname} uses to store cache entries. Typical starting value is p=0.5.
