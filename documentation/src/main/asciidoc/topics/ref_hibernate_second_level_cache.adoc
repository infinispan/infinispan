...
With the divergence of Jakarta packages compared to Java EE, Hibernate ORM 6.x is the only version {brandname} supports
...
.Hibernate ORM 6.6, 7.1 or newer
[source,xml,options="nowrap",subs=attributes+]
----
include::dependencies_maven/hibernate_v6.6.xml[]
----

[NOTE]
====
Hibernate ORM 6.5 is supported with {brandname} 15.2.x and older.
====

.Hibernate ORM 6.2-6.5
[source,xml,options="nowrap",subs=attributes+]
----
...
Apart from {brandname} specific configuration, it is worth noting that enabling second cache requires some changes to the descriptor file
...
By inspecting the statistics you can verify if the cache is being hit and if any new data is stored in cache.
...
_No eviction/expiration is configured for timestamp caches_, nor is it allowed.
...
This is fine because they would all be trying to put the same data loaded from the database.
...
All _entities and collections have initial state transfer disabled_ since there is no need for it.
...
However, replication for query cache only makes sense if, and only if, all of these conditions are true:
...
It is important to note that due to how {brandname} deals with asynchronous replication, stale queries might be found even when the query is done right after an entity/collection update on the same node.
...
If there is only a single JVM accessing the DB, you can use more performant `infinispan-config-local.xml` by setting the `hibernate.cache.infinispan.cfg` property.
...
With the above configuration, you are overriding whatever eviction/expiration settings were defined for the default entity cache name in the {brandname} cache configuration used.
This happens regardless of whether it is the default one or user defined.
More specifically, the following is defined:
...
This allows overrides that only affect a particular entity (such as `com.acme.Person`) or collection type (such as `com.acme.Person.addresses`).
Example:
