...
NOTE: If the `introspection-url` uses HTTPS, it may be necessary to define a link:{server_docs}#configuring-encryption[server identity] containing an appropriate truststore and reference it in the `oauth2-introspection.client-ssl-context` configuration attribute.
