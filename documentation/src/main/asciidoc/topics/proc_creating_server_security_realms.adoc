...
You can define multiple security realms in an XML, a JSON, or a YAML file. The following examples demonstrate defining multiple security realms in each file type.

[NOTE]
====
When defining the name of your security realm in the `name` field, ensure that you do not include any special characters in the name, such as a hyphen (-), an underscore) (_), and so on. By using such characters, you might cause issues with an authentication mechanism that uses the `REST` endpoint or cause connections issues with the CLI.  
====
