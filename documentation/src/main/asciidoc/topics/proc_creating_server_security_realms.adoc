...
[IMPORTANT]
====
Do not add special characters, such as hyphens (`-`) or ampersands (`&`), to security realm names.  
{brandname} Server endpoints can become unreachable if security realm names contain special characters.
====
+
