...
//Community
ifdef::community[]
See the {brandname} Server README, available in the distribution, for descriptions of each folder in your `{server_home}` directory as well as system properties you can use to customize the filesystem.
endif::community[]
//Downstream
ifdef::downstream[]
See the link:{server_docs}#server_readme[{brandname} Server README] for descriptions of each folder in your `{server_home}` directory as well as system properties you can use to customize the filesystem.
endif::downstream[]
====
