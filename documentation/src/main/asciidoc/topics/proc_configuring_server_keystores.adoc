...
When using client hostname validation, according to the rules defined by the https://datatracker.ietf.org/doc/html/rfc2818[RFC 2818] specification, server certificates should include the `subjectAltName` extension of type `dNSName` and/or `iPAddress`.
