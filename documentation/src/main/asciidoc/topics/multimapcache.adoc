...
link:{javadocroot}/org/infinispan/multimap/api/embedded/MultimapCache.html[MultimapCache API] exposes several methods to interact with the Multimap Cache.
...
Asynchronous that returns a view collection of the values associated with key in this multimap cache, if any. Any changes to the retrieved collection do not change the values in this multimap cache.
...
Asynchronous that returns the number of key-value pairs in the multimap cache. It does not return the distinct number of keys.
