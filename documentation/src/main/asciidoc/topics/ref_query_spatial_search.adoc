...
For each of them, the same entity must have two double-typed fields:
...
----
<1> For remote queries, use `org.infinispan.api.annotations.indexing.model.LatLng` as type for any spatial field.
<2> Annotate the spatial fields with `@GeoField`.
...
----
<1> Selects all the restaurants within 100 meters from a given center, in this case 41.91, 12.46.
...
----
<1> Selects all the restaurants within 0.1 Km from a given center, in this case 41.91, 12.46.
<2> Parameters can be extracted for all the values passed to the `within circle` predicate.
====
...
----
<1> Selects all the restaurants contained in the given box.
====
...
----
<1> Selects all the restaurants contained in the polygon identified by the provided vertex points.
====
