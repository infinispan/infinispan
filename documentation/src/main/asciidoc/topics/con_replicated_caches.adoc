...
* Synchronous replication blocks the caller (for example, on a `cache.put(key, value)`) until the modifications have been replicated successfully to all the nodes in the cluster.
