...
|Read timeout in milliseconds for operations that are expected to take considerably longer than a regular single key operation, such as administration operations, bulk operations (`getAll`, `putAll`, `clear`, `size`), queries, iteration and server task execution. Operations that are not long running keep using `socket_timeout`.
|long
|`60000` (1 minute)
...
|`protocol_version`
|Hot Rod protocol version to use. Format: `MAJOR.MINOR` (for example,`3.1`).
|String
...
|`key_store_type`
|Type of the key store (for example,`JKS`, `PKCS12`).
|String
|JVM default
...
|`trust_store_type`
|Type of the trust store (for example,`JKS`, `PKCS12`).
|String
|JVM default
...
|`ssl_protocol`
|SSL/TLS protocol version (for example,`TLSv1.3`, `TLSv1.2`).
|String
|JVM default

|`ssl_provider`
|SSL provider to use (for example,`JDK`, `OPENSSL`).
|String
|JVM default
...
|`ssl_named_groups`
|Comma-separated list of SSL named groups to enable.
|String
|JVM default
...
|`sasl_mechanism`
|SASL mechanism to use for authentication (for example,`SCRAM-SHA-512`, `DIGEST-SHA-256`, `PLAIN`, `EXTERNAL`).
|String
|none
...
|SASL mechanism-specific properties. Replace `<name>` with the property name (for example,`sasl_properties.qop`).
|String
|none
|===
...
|Define a cluster by name with server addresses (for example,`cluster.NYC=nyc1:11222;nyc2:11222`).
|String
|none
