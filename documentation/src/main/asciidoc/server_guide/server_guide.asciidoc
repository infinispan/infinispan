...
With the configuration above, the Single Port Router will operate on `rest` socket
binding. The Single Port will use `SSLRealm1` for TLS/ALPN and will be able to switch
between Rest Server named `rest` and Hot Rod Server named `hotrod`.
