...
==== Using Service Name Indication

link:$$https://en.wikipedia.org/wiki/Server_Name_Indication$$[SNI] might be used to authenticate multiple clients against a single Hot Rod server.

Usually the configuration requires multiple security realms as shown below:

.Security Realms configuration for SSL
[source,xml]
----
<security-realm name="Realm1">
    <server-identities>
        <ssl>
            <keystore path="keystore_server1.jks" relative-to="jboss.server.config.dir" keystore-password="secret" />
        </ssl>
    </server-identities>
</security-realm>
<security-realm name="Realm2">
    <server-identities>
        <ssl>
            <keystore path="keystore_server2.jks" relative-to="jboss.server.config.dir" keystore-password="secret" />
        </ssl>
    </server-identities>
</security-realm>
----

Now, newly configured security realms can be used for SNI configuration:
...
<hotrod-connector socket-binding="hotrod" cache-container="default">
   <authentication security-realm="ApplicationRealm">
      <sni host-name="domain1" security-realm="Realm1" />
      <sni host-name="domain2" security-realm="Realm2" />
   </authentication>
</hotrod-connector>
----

Finally, the clients needs to specify SNI host name as shown below:
...
Note that `sniHostName` and must match SNI configuration from the server.
