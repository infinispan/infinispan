= Upgrading to {brandname} {infinispanversion}
...
This guide walks you through the process of upgrading {brandname}.
...
{brandname} now contains a new implementation of both `AtomicMap` and `FineGrainedAtomicMap`, but the semantics has been preserved. The new implementation does not use `DeltaAware` interface but the link:../user_guide/user_guide.html#functional_map_api[Functional API] instead.
...
In {brandname} 9.1 partition handling has been improved to allow for automatic
...
Up to {brandname} 8.x, the default cache always implicitly existed, even if not declared in the XML configuration.
...
Internally {brandname} 9.x has introduced many improvements to its marshalling codebase in order to improve performance
and allow for greater flexibility. Consequently, data marshalled and persisted by {brandname} 8.x is no longer compatible with {brandname} 9.0.
...
In {brandname} 8.x, cloud related configuration were added to `infinispan-core` module. Since 9.0 they were moved to `infinispan-cloud` module.
...
All {brandname}, Spring and Logger dependencies are now in the `provided` scope. One can decide whether to use small jars or uber jars but they need to be added to the classpath of the application.
...
Entry Retriever is now deprecated and will be removed in {brandname} 9.  This is replaced by the new Streams feature.

=== Map / Reduce is deprecated
Map reduce is now deprecated and will be removed in {brandname} 9.  This is replaced by the new Streams feature.
...
Over time {brandname} added 2 interfaces with identical names and almost identical methods.  The org.infinispan.notifications.KeyFilter and org.infinispan.persistence.spi.AdvancedCacheLoader$KeyFilter interfaces.

Both of these interfaces are used for the sole purpose of filtering an entry by it's given key.  {brandname} 7.0 has also introduced the KeyValueFilter which is similar to both but also can filter on the entries value and/or metadata.
...
The XML schema for the embedded configuration has changed to more closely follow the server configuration. Use the `config-converter.sh` or `config-converter.bat` scripts to convert an {brandname} 6.0 to the current format.
...
Cache Loader and Store configuration has changed greatly in {brandname} 5.2. Please refer to the link:../user_guide/user_guide.html#cache_loaders_and_stores[Cache Loaders and Stores] documentation.

=== Virtual Nodes and Segments

The concept of Virtual Nodes doesn't exist anymore in {brandname} 5.2 and has been replaced by Segments. Please refer to the link:../user_guide/user_guide.html#clustering_modes[Clustering modes] documentation for details.
...
=== Configuration
If you use XML to configure {brandname}, you shouldn't notice any change, except a much faster startup, courtesy of the link:http://en.wikipedia.org/wiki/StAX[StAX] based parser. However, if you use programmatic configuration, read on for the important differences.
