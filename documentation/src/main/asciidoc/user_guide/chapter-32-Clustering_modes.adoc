...
link:http://docs.jboss.org/infinispan/{infinispanversion}/configdocs/infinispan-config-{infinispanversion}.html[`<[replicated|distributed|invalidation]-cache />`]
...
link:http://docs.jboss.org/infinispan/{infinispanversion}/configdocs/infinispan-config-{infinispanversion}.html[`<transport executor="custom" />`]
...
keys throw `AvailabilityException`.
The difference between the two modes is that the cluster will never recover
from unavailable mode by itself.
There are two ways to exit unavailable mode: restart the entire cluster,
or by forcing the availability mode back to AVAILABLE
...
a cache from degraded or unavailable state back to available (at the cost of
consistency).
