...
==== JDBC Migrator
The JDBC Mixed and Binary stores, available in previous versions of {brandname} have been removed due to the poor performance associated with storing entries in buckets.
...
`StoreMigrator.java`, that reads data from an existing Mixed/Binary store and then stores it in a string keyed table via the
<<_JDBC_String_based_Cache_Store, JdbcStringBasedStore>>.

NOTE: New versions might make changes to the persistent storage format. Refer to the link:../upgrading/upgrading.html[upgrading guide]
to see if you need to use the JDBC Migrator.
