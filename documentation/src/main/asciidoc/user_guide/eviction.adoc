...
IMPORTANT: that eviction occurs on a _local_ basis, and is not cluster-wide.  Each
...
link:http://docs.jboss.org/infinispan/{infinispanversion}/configdocs/infinispan-config-{infinispanversion}.html[`<eviction />`]
element to your `<*-cache />` configuration sections or using
link:{javadocroot}/org/infinispan/configuration/cache/EvictionConfigurationBuilder.html[EvictionConfigurationBuilder]
...
[options="header"]
|===============
|Supplied size|Supplied strategy|Example|Eviction behaviour
|-|-| `<eviction />` |no eviction
|&gt; 0|-| `<eviction size="100" />` |the strategy defaults to LIRS and eviction takes place
|&gt; 0|NONE| `<eviction size="100" strategy="NONE" />` | the strategy defaults to LIRS and eviction takes place
|&gt; 0| != NONE | `<eviction size="100" strategy="LRU" />` |eviction takes place with defined strategy
|0|-| `<eviction size="0" />` | no eviction
|0| NONE | `<eviction size="0" strategy="NONE" />` | no eviction
|0| != NONE | `<eviction size="0" strategy="LRU" />` | ConfigurationException
|&lt; 0|-| `<eviction size="-1" />` | no eviction
|&lt; 0| NONE | `<eviction size="-1" strategy="NONE" />` | no eviction
|&lt; 0| != NONE | `<eviction size="-1" strategy="LRU" />` |ConfigurationException
...
.  Four eviction strategies are shipped, link:{javadocroot}/org/infinispan/eviction/EvictionStrategy.html#NONE[EvictionStrategy.NONE] , link:{javadocroot}/org/infinispan/eviction/EvictionStrategy.html#LRU[EvictionStrategy.LRU] , link:{javadocroot}/org/infinispan/eviction/EvictionStrategy.html#UNORDERED[EvictionStrategy.UNORDERED] , and link:{javadocroot}/org/infinispan/eviction/EvictionStrategy.html#LIRS[EvictionStrategy.LIRS] .
...
               .size(2000).expiration().wakeUpInterval(5000l).lifespan(1000l).maxIdle(500l)
               .build();
...
<eviction strategy="LRU" size="1000000000" type="MEMORY"/>
<store-as-binary enabled="true"/>
----
