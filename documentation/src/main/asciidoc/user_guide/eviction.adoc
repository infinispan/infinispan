...
The expiration manager processes expirations by causing the data container to purge expired entries and then causing any cache stores to purge expired entries.
