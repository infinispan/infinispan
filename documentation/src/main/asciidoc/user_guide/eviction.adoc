...
When an entry expires it resides in the data container or cache store
until it is accessed again by a user request. A reaper is also available to check for expired entries and remove them. The reaper runs at a configurable interval of milliseconds and is always enabled when a cache store is present.
//dnaro: 'always enabled when a cache store is present' - Need to clarify this a bit maybe. By default the expiration reaper is enabled when a cache store...

[NOTE]
====
When using a maximum idle time in a clustered cache, you should always enable the reaper. For more information, see link:#max_idle_clustered[Clustered Max Idle].
====
...
With _expiration_ you set _time criteria_ for entries, _how long you want to keep them_ in the cache.
...
In clustered environments, nodes in the cluster can have different access times for the same entry. Entries do not expire from the cache until they reach the maxium idle time for all owners across the cluster.

When a node detects that an entry has reached the maximum idle time and is expired, the node gets the last time that the entry was accessed from the other owners in the cluster. If the other owners indicate that the entry is expired, that entry is not returned to the user and removed from the cache.
//dnaro: Returned to the user? Maybe returned to the requester?
...
* When the expiration reaper finds entries that are expired with the maximum idle time, the reaper updates all nodes to the most recent access time instead of updating the last access time to the current time.
//dnaro: Why would it update the last access time to the current time? Wouldn't that make the entry not expired?
//dnaro: The reaper updates the expired entries across all nodes so that the access time for the entries is the most recent time?
* Clustered transactional caches do *not* remove entries that are expired with the maximum idle time on get operations. These expired entries are removed with the expiration reaper thread only, otherwise deadlocking can occur.
* Iteration across a clustered cache returns entries that might be expired with the maximum idle time. This behavior ensures performance because no remote invocations done during the iteration. However this does not refresh any expired entries, which are removed by the expiration reaper or when accessed directly (`Cache.get`).

[IMPORTANT]
====
* Clustered caches should always use the expiration reaper with the `maxIdle` configuration.
//dnaro: Feel like we should expand this to be more explicit with the reasons why.
...
* Entries that are transferred to the cache via state transfer are created on the new node with a last access time that is equal to the current time. As a result those entries are refreshed.
====
