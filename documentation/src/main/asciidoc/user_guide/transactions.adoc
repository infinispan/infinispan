...
* `write-skew` - enables write skew checks. Check section <<tx:write-skew>> for more details. Default is `true`. Defaults to `false` automatically for the `READ_COMMITTED` isolation level. This attribute is deprecated.
