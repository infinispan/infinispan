...
link:#query.dsl[the Query DSL section]. Then you'll need to obtain the ContinuousQuery (_org.infinispan.query.api.continuous.ContinuousQuery_)
...
API, {brandname} defines its own new link:#query.dsl[query language], based on an internal DSL that is easily
...
==== Storing Protobuf encoded entities[[storing.protobuf]]
...
is detailed in the <<query:configuration-api,Querying {brandname}>> chapter.
...
embedded mode which is covered in link:#query.dsl[this] section.
