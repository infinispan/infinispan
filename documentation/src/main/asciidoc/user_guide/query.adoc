...
Querying is possible both in <<query.library,library>> and <<query.remote,client/server mode>> (for Java, C#, Node.js and other clients), and {brandname} can index data
...
Apart from indexed queries, {brandname} can run queries over non-indexed data (<<query.indexless, indexless queries>>) and over partially indexed data (<<query.hybrid,hybrid queries>>).

In terms of Search APIs, {brandname} has its own query language called _<<query.ickle,Ickle>>_, which is a subset of JP-QL providing extensions for full-text
querying. The <<query.dsl,{brandname} Query DSL>> can be used for both embedded and remote java clients when full-text is not required; for Java embedded clients
{brandname} offers the <<query.hibernatesearch,Hibernate Search Query API>> which supports running Lucene queries in the grid, apart from advanced search capabilities
like Faceted and Spatial search.

Finally, {brandname} has support for <<query.continuous,Continuous Queries>>, which works in a reverse manner to the other APIs: instead of creating, executing a query
...
letting {brandname} <<query.autoconfig,configure the indexing automatically>>:
...
{brandname} supports shared indexes leveraging the <<integrations.adoc#integrations:directory-provider,{brandname} Directory Provider>>, which stores indexes in a separate set of caches. Two index managers are available
to use shared indexes: {brandname}IndexManager and AffinityIndexManager.
...
when {brandname} replicates it to another node. The `ALL` mode is usually associates with <<query.non-shared-index>> in order to create full index replicas on each node.

====== {brandname}IndexManager

This index manager uses the <<integrations.adoc#integrations:directory-provider,{brandname} Directory Provider>>, and is suitable for creating shared indexes. Index mode should be set to `LOCAL` in this configuration.
...
<distributed-cache name="default" >
    <indexing index="LOCAL">
        <property name="default.indexmanager">
            org.infinispan.query.indexmanager.{brandname}IndexManager
        </property>
...
In order to avoid index corruption when two or more nodes of the cluster try to write to the index at the same time, the _{brandname}IndexManager_ internally
...
The AffinityIndexManager is an *experimental* index manager used for shared indexes that also stores indexes using the <<integrations.adoc#integrations:directory-provider,{brandname} Directory Provider>>.
Unlike the {brandname}IndexManager, it does not have a single node (master) that handles all the indexing cluster wide, but rather splits the index using multiple shards, each shard being responsible for indexing data associated with one or more {brandname} segments.
...
{brandname} storage makes use of the <<integrations:lucene-directory, {brandname} Lucene directory>> that saves the indexes to a set of caches; those caches can be configured like any other {brandname} cache, for example by adding a cache store to have indexes persisted elsewhere apart from memory.
...
| default.exclusive_index_use | true | indexing operation in exclusive mode, allowing Hibernate Search to optimize writes
| default.indexmanager | org.infinispan.query.indexmanager.{brandname}IndexManager | Delegates index writing to a single node in the {brandname} cluster
...
===== {brandname} Query DSL [[query.dsl]]

Starting with 6.0 {brandname} provides its own query DSL, independent of Lucene and Hibernate Search.
...
QueryBuilder qb = ...
qb.having("title").eq("{brandname} Data Grid Platform");
----
...
QueryBuilder qb2 = ...
qb2.having("title").like("%{brandname}%");
...
[source,java]
----
// match all books that have the word "{brandname}" in their title
// or have an author named "Manik" and their description contains the word "clustering"
Query query = queryFactory.from(Book.class)
  .having("title").like("%{brandname}%")
...
[source,java]
----
// match all books that do not have the word "{brandname}" in their title and are authored by "Manik"
Query query = queryFactory.from(Book.class)
  .not().having("title").like("%{brandname}%")
...
// the word "{brandname}" or their description contains the word "clustering"
Query query = queryFactory.from(Book.class)
  .having("author.name").eq("Manik");
  .and(queryFactory.having("title").like("%{brandname}%")
...
[source,java]
----
// match all books that have the word "{brandname}" in their title or description
// and return only their title and publication year
Query query = queryFactory.from(Book.class)
  .select("title", "publicationYear")
  .having("title").like("%{brandname}%")
  .or().having("description").like("%{brandname}%"))
  .toBuilder().build();
----
...
API, {brandname} defines its own new link:#infinispan_s_query_dsl[query language], based on an internal DSL that is easily
...
is detailed in the <<query:configuration,Querying {brandname}>> chapter.
...
Book book2 = new Book();
book2.setTile("{brandname} Data Grid Platform");
remoteCache.put(2, book2);

QueryFactory qf = Search.getQueryFactory(remoteCache);
Query query = qf.from(Book.class)
            .having("title").like("%{brandname}%").toBuilder()
            .build();
...
By default, the <<query.index-manager, Index Managers>> work in sync mode, meaning when data is written to {brandname}, it will perform the indexing operations synchronously.
