...
For example, in a cache paramerized as `Cache<String, String>`, `Cache.put(String key, String value)` returns a `String`.
...
==== DecoratedCache
Another approach would be to use the link:$$http://docs.jboss.org/infinispan/5.1/apidocs/org/infinispan/DecoratedCache.html$$[DecoratedCache] wrapper.
...
===  The Grouping API
In some cases you may wish to co-locate a group of entries onto a particular node.
In this, the group API will be useful for you.
