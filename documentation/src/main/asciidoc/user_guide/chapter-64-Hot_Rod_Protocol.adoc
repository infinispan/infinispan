...
|==============================================================================
| Field Name          | Size       | Value
| Segments size       | vInt       |  Size of the bitset encoding of the segments ids to iterate on. The size is the maximum segment id rounded to nearest multiple of 8.
| Segments            | byte array | (Optional) the segments ids bitset encoded, where each bit with value 1 represents a segment in the set. Byte order is little-endian. +
...
More details in the java.util.BitSet implementation
| FilterConverter     | String      | (Optional) KeyValueFilterConverter factory name deployed on the server
