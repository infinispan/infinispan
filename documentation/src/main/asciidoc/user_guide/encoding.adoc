...
| Embedded | StorageType.OFF_HEAP  | GlobalMarshallerEncoder | Uses Infinispan internal marshaller to convert to byte[]. May delegate to the configured marshaller in the cache manager.
