...
include::topics/productized/spring_boot_starter.adoc[]
endif::productized[]
...
[NOTE]
====
{brandname} supports Spring 4 and Spring 5 with different sets of dependencies.
====
...
+
The following is an example with Spring 5 in embedded mode:
+
...
[[spring_disable_jmx_autoconfig]]
====== Avoiding Duplicate MBean Registration
When using Remote client-server mode, you can collect statistics for your Hot Rod client implementation with the `HotRodClient` JMX MBean.

If you enable JMX statistics with the `HotRodClient` MBean, you should configure your Spring application so that it does not automatically register MBeans. Otherwise, both your Spring application and your Hot Rod client implementation attempt to register the MBean and the following exception occurs:

[source,options=nowrap]
----
org.springframework.jmx.export.UnableToRegisterMBeanException: Unable to register MBean
...
InstanceAlreadyExistsException: MXBean already registered with name org.infinispan:type=HotRodClient,name=Default
----

Disable the Spring JMX autoconfig with `spring.jmx.enabled=false`.
...
Add dependencies to your `pom.xml` as follows:

Spring 4::
* Embedded mode: `infinispan-spring4-embedded`
* Remote client-server mode: `infinispan-spring4-remote`
* Spring Framework: `spring-session`
+
The following is an example with Spring 4 in embedded mode:
+
include::topics/dependencies/spring4_session_dependencies.adoc[]
+
Spring 5::
* Embedded mode: `infinispan-spring5-embedded`
* Remote client-server mode: `infinispan-spring5-remote`
* Spring Framework: `spring-session-core`
+
The following is an example with Spring 5 in remote client-server mode:
+
include::topics/dependencies/spring5_session_dependencies.adoc[]

After you add dependencies, do the following:
