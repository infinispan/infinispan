...
NOTE: This interface is available since {brandname} 7.0.0.
...
The following sections describe how {brandname} handles <<split-brain,split brain>> and
<<successive-node-failures,successive failures>> for each of the partition handling strategies. This is followed by a section
describing how {brandname} allows for automatic conflict resolution upon partition merges via <<merge-policies, merge policies>>.
...
| MergePolicies.NONE
| No attempt is made to resolve conflicts. Instead, entries hosted on the minority partition are removed and the nodes in this partition do not hold any data until the rebalance starts. Note, this behaviour is equivalent to prior Infinispan versions which did not support conflict resolution.
Note, in this case all changes made to entries hosted on the minority partition are lost, but once the rebalance has finished all entries will be consistent.

| Fully qualified class name
| The custom implementation for merge will be used <<partition-handling-custom-merge-policy, Custom merge policy>>
|===
...
[[partition-handling-custom-merge-policy]]
...
In order for a Custom merge policy to be utilised on the server, compatibility mode must be enabled if your policies semantics require access to the stored Key/Value objects. This is because without compatibility mode, cache entries in the server are stored in a marshalled format and the Key/Value objects returned to your policy would be instances of WrappedByteArray.
However, if the custom policy only depends on the metadata associated with a cache entry, then compatibility mode is not required and should be avoided (unless needed for other reasons) due to the additional performance cost of marshalling data per request.
Finally, compatibility mode is never required if one of the provided merge policies is used.
