...
Creating a local-only cache-manager is trivial: just use the _no-argument_ DefaultCacheManager
...
----
You can also create local caches in a clustering-capable CacheManager.
...
.Replication mode
image::images/Figure1_6.png[align="center", title="Replication mode"]
...
.Invalidation mode
image::images/Figure2_5.png[align="center", title="Invalidation mode"]
...
.Distribution mode
image::images/Figure3_3.png[align="center", title="Distribution mode"]
...
.L1 caching
image::images/Figure4_4.png[align="center", title="L1 caching"]
...
to locate the primary copy, the backup copy is on the node that wrote the data last time (or another node
when the data was last written on primary owner).

This has the advantage of single RPC for any write (Distribution Mode requires one or two RPCs), but reads
have to always hit the primary owner. That results in faster writes, and slower reads, and therefore this
...
is not supported either; use asynchronous Cache API instead.
