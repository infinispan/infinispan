...
[cols="20,^15,^15,^15,^15,^15",options="header"]
|============================================================
|                  | Simple      | Local       | Invalidation | Replicated | Distributed
| Clustered        | [red]*No*    | [red]*No*    | [green]*Yes*   | [green]*Yes* | [green]*Yes*
...
stem:[(sum_(i=1)^"nodes""node_capacity")/"owners"]
| Availability     | [red]*Single node*| [red]*Single node*| [red]*Single node*| [green]*All nodes*| [yellow]*Owner nodes*
| Features         | [red]*No TX, persistence, indexing*| [green]*All* | [green]*All* | [green]*All* | [green]*All* |
|============================================================
...
[[conflict-manager]]
==== Conflict Manager
The conflict manager is a tool that allows users to retrieve all stored replica values for a given key. In addition to allowing users
to process a stream of cache entries whose stored replicas have conflicting values. Note that the stream of conflicts is
processed per entry, however it is lazily loaded on a per segment basis.  Conflicts are exposed via a stream, opposed to a
collection, in order to reduce the chances of an OutOfMemoryError occurring.
