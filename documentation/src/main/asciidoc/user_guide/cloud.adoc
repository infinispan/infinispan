...
include::topics/dependencies/ispn_cloud.adoc[]
...
Since OpenShift uses Kubernetes underneath both of them can use either - link:https://github.com/jgroups-extras/jgroups-kubernetes[Kube_PING] or DNS_PING.
...
DNS_PING runs the specified query against the DNS server to get the cluster members list. See link:https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/[here] on how enlist the cluster nodes into DNS
...
in order to use the updated configuration bits. On the other hand the link:#rolling_upgrades[Rolling Upgrade] is a process of migrating data from one {brandname} cluster to another one.
A typical example is migrating from one version to another.

For both Kubernetes and OpenShift, the Rolling Upgrade procedure is almost the same. It is based on a standard link:#rolling_upgrades[Rolling Upgrade procedure] with small changes.
