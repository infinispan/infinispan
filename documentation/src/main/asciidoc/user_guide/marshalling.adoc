...
TIP: Since {brandname} 5.0, marshalling non-Serializable key/value objects are supported as long as users can to provide meaningful Externalizer implementations for these non-Seralizable objects. <<plugging_infinispan_with_user_defined_externalizers, This section>> has more details.
