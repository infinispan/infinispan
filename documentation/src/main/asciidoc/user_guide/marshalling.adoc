...
When using lazy deserialization/storing as binary, keys and values are wrapped as link:{javadocroot}/org/infinispan/marshall/core/MarshalledValue[MarshalledValue]s.
...
* This model requires that the marshalled class be annotated with {@link SerializeWith} but a user might need to provide an Externalizer for a class for which source code is not available, or for any other constraints, it cannot be modified.
