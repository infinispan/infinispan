...
[IMPORTANT]
====
When expiring entries, {brandname} replicates a command that removes expired
entries across clusters. As of this release {brandname} also replicates this 
command across sites, which is not necessary and will be removed in a future
release.
====
