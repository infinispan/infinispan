...
Aside from deleting caches, as described above, you can obviously also create them on the remote system using the `MKCOL` method:
...
===== Admin flags [[rest:adminflags]]
Operations which manipulate the server configuration, such as `MKCOL`, support an `Admin-Flags` header which affect their behaviour.
For example, adding the 'Admin-Flags: PERSISTENT' to a `MKCOL` operation, will make the configuration persistent, if supported by the server. Refer to the link:{javadocroot}/org/infinispan/commons/api/CacheContainerAdmin.AdminFlag.html[CacheContainerAdmin.AdminFlag] javadocs.
