...
===== Configuration
All the configurations are passed to the RemoteCacheManager's constructor as key-value pairs, through an instance of link:{jdkdocroot}/java/util/Properties.html[java.util.Properties] or reference to a .properties file. Please refer to the javadoc of RemoteCacheManager for a exhaustive list of the possible configuration elements.
...
====  Interacting With Hot Rod Server From Within Same JVM [[server-hotrod:embedded]]
...
The most common case is for a client to use a Hot Rod client library directly to store data in the Hot Rod server. In this case, assuming that the client used the existing Java Hot Rod client, the default marshaller used to marshall objects into byte arrays is the link:{javadocroot}/org/infinispan/marshall/jboss/GenericJBossMarshaller.html[GenericJBossMarshaller] . So, if a user wants to read data from the Hot Rod server directly, it would need to execute something along the lines of:
...
image::images/client_server.png[align="center", title="Multiple caching tiers"]
...
While previous Infinispan versions were already providing <<querying_infinispan,indexing and searching>> of Java
entities to embedded clients, starting with Infinispan 6.0 and the introduction of the new
<<hot_rod_protocol_1_3,Hot Rod protocol version 1.3>> we add support for remote, language neutral, querying.
...
that the user belongs to the _'___schema_manager role'_
