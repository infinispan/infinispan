...
==== Using internal/external IP addresses

In some cases (e.g. OpenStack or Kubernetes with services exposed using load balancers) servers use different IP addresses for clustering and different for communicating with the client.
In order to take advantage `hash-distribution-aware` client in such configuration one needs to provide a mapping between those two types of addresses. Specially for this reason we created
`AddressMapper` interface. One is responsible for implementing mapping between internal and external addresses:

[source,java]
----
public class MyAddressMapper implements AddressMapper {
...
The final part is to supply the mapping class into the Hot Rod Client configuration:

[source,java]
----
ConfigurationBuilder builder = new ConfigurationBuilder();
builder.addServer()
 // ... provide at least one server address
