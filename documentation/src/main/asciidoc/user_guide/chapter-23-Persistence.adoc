...
*  `SingleFileStore` is a filesystem-based implementation. Usage on shared filesystems like NFS, Windows shares, etc. should be avoided as these do not implement proper file locking and can cause data corruption. File systems are inherently not transactional, so when attempting to use your cache in a transactional context, failures when writing to the file (which happens during the commit phase) cannot be recovered.

TIP: The `SingleFileStore` is new in Infinispan 6.x and replaces the previous file store implementation offering better performance at the cost of some additional memory consumption(it keeps all the keys in memory at all times).
...
Below is an sample configuration for the link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/persistence/jdbc/stringbased/JdbcStringBasedStore.html$$[JdbcStringBasedStore] . For detailed description of all the parameters used refer to the link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/persistence/jdbc/stringbased/JdbcStringBasedStore.html$$[JdbcStringBasedStore] .
