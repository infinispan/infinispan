...
However, when this option is enabled and concurrent writes are happening at the same time as cluster topology changes,
a concurrent write might override the remove resulting in a non-empty cache.
