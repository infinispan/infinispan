...
TIP: This is also available as a `start` JMX operation on the link:$$http://docs.jboss.org/infinispan/6.0/apidocs/jmxComponents.html#MassIndexer$$[MassIndexer MBean]
...
can be obtained from the Hibernate Search' _SearchFactory_, as demonstrated in the following code snippet.

[source,java]
----
SearchManager searchManager = Search.getSearchManager(cache);
org.hibernate.search.stat.Statistics statistics = searchManager.getSearchFactory().getStatistics();
----
...
(link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/query/dsl/FilterConditionEndContext.html$$[_FilterConditionEndContext_])
...
link:$$https://github.com/infinispan/infinispan/blob/6.0.0.CR1/query/src/test/java/org/infinispan/query/dsl/embedded/QueryDslConditionsTest.java$$[QueryDslConditionsTest]
is a fine example.
