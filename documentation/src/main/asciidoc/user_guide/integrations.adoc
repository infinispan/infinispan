...
`@Enable{brandname}EmbeddedHttpSession` or `@Enable{brandname}RemoteHttpSession` annotations have 2 optional parameters:
...
==== Conclusion
Hopefully you enjoyed our quick tour of {brandname}'s support for Spring's cache and session abstraction and saw how easy it is for all your caching woes to be taken care of by {brandname}. More information may be found in Spring's link:http://docs.spring.io/spring-framework/docs/4.1.1.RELEASE/spring-framework-reference/html/cache.html[reference documentation]. Also see link:http://spring.io/blog/2011/02/23/spring-3-1-m1-cache-abstraction[this link] - a very nice posting on the official Spring blog for a somewhat more comprehensive introduction to Spring's cache abstraction.

===  {brandname} modules for WildFly

As the {brandname} modules shipped with link:http://wildfly.org/[Wildfly application server] are tailored to its internal usage, it is recommend to install separate modules
if you want to use {brandname} in your application that is deployed to Wildfy. By installing these modules, it is possible to deploy user applications without packaging the {brandname} JARs within the deployments (WARs, EARs, etc), thus minimizing their size.
...
This dependency will be available once the {brandname} modules are <<_Modules_installation_section,installed>>.
