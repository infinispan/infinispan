...
* JMX over JBoss Remoting is the protocol of choice when your {brandname} application is running inside {wildflybrandname}.
...
[IMPORTANT]
====
{brandname} CLI sessions expire if they remain idle for more than six minutes. Running commands after the session expires results in the following message:
...
include::topics/proc_starting_stopping_endpoints.adoc[leveloffset=+3]
