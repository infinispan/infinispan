                     ...
                     (stream) -> {
                        ...
                        CompletableFuture.runAsync(() -> {
                           for (String name : metadataManager.getProtofileNames()) {
                              String schema = metadataManager.getProtofile(name);
                              String errors = metadataManager.getFileErrors(name);
                              stream.sendEvent(new ServerSentEvent("create-schema", "name: " + name + "\nerrors: " + errors + "\n" + schema));
                           }
                        }, invocationHelper.getExecutor());
         ...
         if (event.getConfigurationEventType() == ConfigurationChangedEvent.EventType.REMOVE) {
            ...
            switch (event.getConfigurationEntityType()) {
               ...
               case ConfigurationChangedEvent.SCHEMA:
                  ...
                  String response = "name: " + entityName + "\nerrors: " + value.get("errors") + "\n" + value.get("file");
                  sse = new ServerSentEvent(eventType, response);
                  break;
               default:
