   ...
   public Invocations getInvocations() {
      return new Invocations.Builder("cache", "Cache API")
            ...
            .invocation().methods(DELETE).path("/v3/caches/{cacheName}/_delete-by-query")
               ...
               .response(NOT_FOUND, CACHE_NOT_FOUND_RESPONSE, TEXT_PLAIN, Schema.STRING)
               .handleWith(queryAction::deleteByQuery)
            .invocation().methods(POST).path("/v3/caches/{cacheName}/_update-by-query")
               .operationId("updateByQuery")
               .name("Update entries in a cache matching an Ickle query")
               .permission(AuthorizationPermission.BULK_WRITE)
               .parameter("cacheName", ParameterIn.PATH, true, Schema.STRING, "The cache name")
               .parameter("local", ParameterIn.QUERY, false, Schema.BOOLEAN, "Restrict operation to the local node")
               .request("The query request object", true, Map.of(APPLICATION_JSON, new Schema(JsonQueryRequest.class)))
               .response(OK, "The number of updated entries", APPLICATION_JSON, Schema.LONG)
               .response(NOT_FOUND, CACHE_NOT_FOUND_RESPONSE, TEXT_PLAIN, Schema.STRING)
               .handleWith(queryAction::updateByQuery)
            .invocation().methods(PUT).path("/v3/caches/{cacheName}/_update-by-query")
               .operationId("updateByQueryPut")
               .name("Update entries in a cache matching an Ickle query (PUT)")
               .permission(AuthorizationPermission.BULK_WRITE)
               .parameter("cacheName", ParameterIn.PATH, true, Schema.STRING, "The cache name")
               .parameter("local", ParameterIn.QUERY, false, Schema.BOOLEAN, "Restrict operation to the local node")
               .request("The query request object", true, Map.of(APPLICATION_JSON, new Schema(JsonQueryRequest.class)))
