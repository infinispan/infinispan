...
import static org.infinispan.commons.util.InfinispanCollections.sortedMapOf;
   ...
   public Invocations getInvocations() {
      return new Invocations.Builder("cache", "Cache API")
            ...
            .invocation().methods(GET, HEAD).path("/v3/caches/{cacheName}/config")
               ...
               .response(OK, "Cache configuration as XML", APPLICATION_XML, Schema.STRING)
               .response(OK, "Cache configuration as JSON", APPLICATION_JSON, Schema.STRING)
               .response(OK, "Cache configuration as YAML", APPLICATION_YAML, Schema.STRING)
               .response(NOT_FOUND, CACHE_NOT_FOUND_RESPONSE, TEXT_PLAIN, Schema.STRING)
            ...
            .invocation().methods(POST, PUT).path("/v3/caches/{cacheName}")
               ...
               .request("The cache configuration", true,
                  // Unfortunately, the openapi-generator cannot handle multiple request types yet, and only picks the first.
                  // We therefore give priority to JSON
                  // https://github.com/OpenAPITools/openapi-generator/issues/3990
                  sortedMapOf(
            ...
            .invocation().method(DELETE).path("/v3/caches/{cacheName}")
               ...
               .response(OK, "The cache was removed")
               .response(NOT_FOUND, CACHE_NOT_FOUND_RESPONSE, TEXT_PLAIN, Schema.STRING)
            ...
            .invocation().methods(HEAD).path("/v3/caches/{cacheName}/rolling-upgrade/source-connection")
               ...
               .response(OK, "The cache is connected to a source cluster")
               .response(NOT_FOUND, "The cache does not exists or is not connected to a source cluster")
            ...
            .invocation().method(POST).path("/v3/caches/{cacheName}/_initialize")
               ...
               .response(BAD_REQUEST, "Illegal state")
               .response(NOT_FOUND, CACHE_NOT_FOUND_RESPONSE, TEXT_PLAIN, Schema.STRING)
