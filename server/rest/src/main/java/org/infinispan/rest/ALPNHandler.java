...
public class ALPNHandler extends ApplicationProtocolNegotiationHandler {
   ...
   public ALPNHandler(RestServer restServer, Map<String, ProtocolServer<?>> upgradeServers) {
   ...
   public void configurePipeline(ChannelHandlerContext ctx, String protocol) {
      configurePipeline(ctx.pipeline(), protocol, restServer, upgradeServers);
   }
   ...
   private static void addCommonHandlers(ChannelPipeline pipeline, RestServer restServer) {
      ...
      // Handles CORS
      if (!restServer.getConfiguration().getCorsRules().isEmpty()) {
         pipeline.addLast(new CorsHandler(restServer.getConfiguration().getCorsRules(), true));
      }
      // Handles Keep-alive
