...
public final class TopKListFunction
   ...
   public List<Object> apply(EntryView.ReadEntryView<byte[], Object> view) {
      ...
      List<Object> items = topK.list(withCount);
      // Convert byte[] items to String for RESP serialization (Resp3Type.AUTO handles String but not byte[])
      for (int i = 0; i < items.size(); i += withCount ? 2 : 1) {
         items.set(i, new String((byte[]) items.get(i)));
      }
      return items;
   }
}
