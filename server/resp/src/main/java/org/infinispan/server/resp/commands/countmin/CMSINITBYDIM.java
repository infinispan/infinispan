...
public class CMSINITBYDIM extends RespCommand implements Resp3Command {
   ...
   public CompletionStage<RespRequestHandler> perform(Resp3Handler handler, ChannelHandlerContext ctx,
      ...
      try {
         width = ArgumentUtils.toInt(arguments.get(1));
      } catch (NumberFormatException e) {
         handler.writer().customError(ProbabilisticErrors.CMS_INVALID_WIDTH);
         return handler.myStage();
      }

      try {
         depth = ArgumentUtils.toInt(arguments.get(2));
      } catch (NumberFormatException e) {
         handler.writer().customError(ProbabilisticErrors.CMS_INVALID_DEPTH);
         return handler.myStage();
      }
