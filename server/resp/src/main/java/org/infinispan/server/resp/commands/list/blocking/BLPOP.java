...
public class BLPOP extends RespCommand implements Resp3Command {
   ...
   public CompletionStage<RespRequestHandler> perform(Resp3Handler handler,
      ...
      var pollStage = pollAllKeys(listMultimap, filterKeys);
   ...
   CompletionStage<Collection<byte[]>> addSubscriber(EmbeddedMultimapListCache<byte[], byte[]> listMultimap,
      ...
      PubSubListener pubSubListener = new PubSubListener(filterKeys, handler, cache, listMultimap);
   ...
   public static class PubSubListener {
      ...
      private PubSubListener(List<byte[]> filterKeys, Resp3Handler handler, AdvancedCache<byte[], Object> cache,
         ...
         this.synchronizer = new PollListenerSynchronizer(filterKeys, mml);
   ...
   public static class PollListenerSynchronizer {
      ...
      private PollListenerSynchronizer(List<byte[]> keys, EmbeddedMultimapListCache<byte[], byte[]> multimapList) {
      ...
      private void runPoll(Object key) {
         if (canPollJustEventKey && key != this) {
            BLPOP.pollKeyValue(multimapList, (byte[])key).whenComplete(whenCompleteConsumer);
         } else {
            BLPOP.pollAllKeys(multimapList, keys).whenComplete(whenCompleteConsumer);
         }
      }
      ...
      private void onListenerAdded() {
         ...
         if (emptyQueue) {
            BLPOP.pollAllKeys(multimapList, keys).whenComplete((v, t) -> {
               whenCompleteConsumer.accept(v, t);
            });
         }
      }
