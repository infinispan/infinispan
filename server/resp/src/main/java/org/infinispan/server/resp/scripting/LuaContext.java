...
public class LuaContext implements AutoCloseable {
   ...
   private final static String[] LIBRARIES_ALLOW_LIST = {"string", "cjson", "bit", "cmsgpack", "math", "table", "struct", "os"};
   ...
   private void registerRedisAPI() {
      ...
      tableAdd(lua, "setresp", l -> {
         ...
         long resp = l.toInteger(-argc);
         if (resp != 2 && resp != 3) {
            l.error("RESP version must be 2 or 3.");
         }
         // As Infinispan doesn't handle RESP 2, should we throw an error here ?
         return 0;
      });
   ...
   public int executeRespCommand(Lua l, boolean raiseError) {
      ...
      CompletableFuture<RespRequestHandler> future = ((Resp3Command) respCommand).perform(handler, ctx, args).toCompletableFuture();
      try {
         future.get();
    ...
    */
   static void luaPushError(Lua lua, String error) {
      String msg;
   ...
   private static int luaSetErrorMetatable(Lua lua) {
