...
public class BFINFO extends RespCommand implements Resp3Command {
   ...
   public CompletionStage<RespRequestHandler> perform(Resp3Handler handler, ChannelHandlerContext ctx,
      ...
      CompletionStage<BloomFilterInfo> result = cache.eval(key, BloomFilterInfoFunction.INSTANCE);

      InfoType finalInfoType = infoType;
      return handler.stageToReturn(result, ctx, (info, w) -> {
         Map<String, Long> map = info.toMap(finalInfoType);
         if (finalInfoType == InfoType.ALL) {
            w.map(map, Resp3Type.SIMPLE_STRING, Resp3Type.INTEGER);
         } else {
