...
df -h > "$TMPDIR/df"
lsof -itcp -P -n -b | grep "LISTEN"  > "$TMPDIR/ss-tcp"
lsof -iudp -P -n -b > "$TMPDIR/ss-udp"
lsof -b / > "$TMPDIR/lsof" 2> /dev/null
...
"$JAVA_HOME"/bin/jstack "$SERVER_PID" > "$TMPDIR/$SERVER_PID/thread-dump"
