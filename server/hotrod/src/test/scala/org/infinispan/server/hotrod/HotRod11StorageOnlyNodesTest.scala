...
class HotRod11StorageOnlyNodesTest extends HotRodMultiNodeTest {
   ...
   def testAddingStorageOnlyNode(m: Method) {
      ...
      try {
         ...
         TestingUtil.waitForRebalanceToComplete(cache(0, cacheName), cache(1, cacheName), cache(2, cacheName))
         ...
         TestingUtil.waitForRebalanceToComplete(cache(0, cacheName), cache(2, cacheName))
         ...
         TestingUtil.waitForRebalanceToComplete(cache(0, cacheName))
      }
