...
class HotRodSharedContainerTest extends MultipleCacheManagersTest {
   ...
   def testIncompatibleTopologyConfiguration() {
      val basePort = UniquePortThreadLocal.get.intValue
      hotRodServer1 = startHotRodServer(cacheManagers.get(0), basePort, new HotRodServerConfigurationBuilder().topologyStateTransfer(true))
      hotRodServer2 = startHotRodServer(cacheManagers.get(0), basePort + 50, new HotRodServerConfigurationBuilder().topologyStateTransfer(false))
   }

   @AfterClass(alwaysRun = true)
   override def destroy() {
      log.debug("Test finished, close servers, and cache managers")
