...
object Decoder10 extends AbstractVersionedDecoder with ServerConstants with Log {
   ...
   override def readHeader(buffer: ChannelBuffer, version: Byte, messageId: Long, header: HotRodHeader): Boolean = {
      ...
      val (op, endOfOp) = streamOp match {
         ...
         case 0x1D => (BulkGetKeysRequest, true)
   ...
   override def customReadHeader(h: HotRodHeader, buffer: ChannelBuffer, cache: Cache[ByteArrayKey, CacheValue]): AnyRef = {
      h.op match {
         ...
         case BulkGetKeysRequest => new BulkGetKeysResponse(h.version, h.messageId, h.cacheName,
                  h.clientIntel, BulkGetKeysResponse, Success, h.topologyId)
      }
   }
