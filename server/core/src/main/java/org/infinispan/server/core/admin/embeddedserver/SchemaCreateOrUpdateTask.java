...
public class SchemaCreateOrUpdateTask extends AdminServerTask<byte[]> {
   ...
   protected byte[] execute(EmbeddedCacheManager cacheManager, Map<String, List<String>> parameters, EnumSet<CacheContainerAdmin.AdminFlag> flags) {
      ...
      boolean force = Boolean.parseBoolean(getParameter(parameters, FORCE));
      ...
      switch (op) {
         case C -> result = create(schemasCache, schemaName, schemaContent);
         case U -> result = update(schemasCache, schemaName, schemaContent, force);
         case S -> result = save(schemasCache, schemaName, schemaContent, force);
      }
   ...
   private String update(AdvancedCache<String, String> schemas,
      ...
      String actualSchemaContent = existingEntry != null ? existingEntry.getValue() : null;
