...
public class RollingUpgradeHandlerXSiteExtension extends InfinispanXSiteServerExtension {
   ...
   private RollingUpgradeHandlerXSiteExtension(Map<String, InfinispanServerExtensionBuilder> sites,
                                              String fromVersion, String toVersion) {
      super(new ArrayList<>());
      this.sites = sites.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, e ->
         RollingUpgradeHandlerExtension.convertBuilder(e.getValue(), fromVersion, toVersion).build()));
   }

   public static RollingUpgradeHandlerXSiteExtension from(InfinispanXSiteServerExtensionBuilder builder,
                                                          String fromVersion, String toVersion) {
      return new RollingUpgradeHandlerXSiteExtension(builder.siteConfigurations(), fromVersion, toVersion);
   }
