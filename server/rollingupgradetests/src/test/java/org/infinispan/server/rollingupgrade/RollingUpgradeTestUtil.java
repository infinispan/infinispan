...
public class RollingUpgradeTestUtil {
   ...
   public static RollingUpgradeVersion getFromVersion() {
      String label = normalizeImageReference(System.getProperty(INFINISPAN_ROLLING_UPGRADE_FROM_VERSION, "latest"));
   ...
   public static RollingUpgradeVersion getToVersion() {
      String toVersion = normalizeImageReference(System.getProperty(INFINISPAN_ROLLING_UPGRADE_TO_VERSION));
      if (toVersion != null) {
   ...
   static String normalizeImageReference(String value) {
      if (value != null && !value.startsWith("image://") && !value.startsWith("file://") && value.contains("/")) {
         return "image://" + value;
      }
      return value;
   }
   ...
   static String extractVersionFromImageLabel(String label) {
      ...
      int colonIdx = s.indexOf(':', Math.max(slashIdx, 0));
      if (colonIdx >= 0) {
