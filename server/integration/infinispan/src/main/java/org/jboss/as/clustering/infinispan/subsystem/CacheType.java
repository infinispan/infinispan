 ...
 */
enum CacheType {
    LOCAL(ModelKeys.LOCAL_CACHE, new LocalCacheAdd(), new CacheRemoveHandler()),
    DISTRIBUTED(ModelKeys.DISTRIBUTED_CACHE, new DistributedCacheAdd(), LOCAL.getRemoveHandler()),
    REPLICATED(ModelKeys.REPLICATED_CACHE, new ReplicatedCacheAdd(), LOCAL.getRemoveHandler()),
    INVALIDATION(ModelKeys.INVALIDATION_CACHE, new InvalidationCacheAdd(), LOCAL.getRemoveHandler()),
    ;
    ...
    private final CacheAdd addHandler;
    private final CacheRemoveHandler removeHandler;

    private CacheType(String key, CacheAdd addHandler, CacheRemoveHandler removeHandler) {
        this.key = key;
