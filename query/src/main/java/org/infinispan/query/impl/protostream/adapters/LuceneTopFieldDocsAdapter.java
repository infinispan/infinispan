...
public class LuceneTopFieldDocsAdapter {
   ...
   @ProtoFactory
   static TopFieldDocs protoFactory(TotalHits totalHits, Stream<WrappedMessage> scoreDocs, SortField[] sortFields) {
      ScoreDoc[] docs = scoreDocs.map(WrappedMessages::unwrap).toArray(ScoreDoc[]::new);
   ...
   @ProtoField(2)
   Stream<WrappedMessage> getScoreDocs(TopFieldDocs topFieldDocs) {
      // We must use a WrappedMessage here to allow for inheritance as this can either be FieldDoc or ScoreDoc
      return Stream.of(topFieldDocs.scoreDocs).map(WrappedMessages::orElseNull);
   }
