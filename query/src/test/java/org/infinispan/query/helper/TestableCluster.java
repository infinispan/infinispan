...
public class TestableCluster<K, V> {
   ...
   public synchronized EmbeddedCacheManager startNewNode() {
      ...
      caches.add(cache);
      waitForStateTransferToComplete(cache, caches);
      return cacheManager;
   }
   ...
   private static <K, V> void waitForStateTransferToComplete(Cache<K, V> cache, List<Cache<K, V>> caches) {
      if (cache.getCacheConfiguration().clustering().cacheMode() != CacheMode.LOCAL) {
         TestingUtil.waitForStateTransferToComplete(caches);
      }
   }
