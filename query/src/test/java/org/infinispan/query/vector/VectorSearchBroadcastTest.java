...
public class VectorSearchBroadcastTest extends MultipleCacheManagersTest {
   ...
   @Test
   public void test() {
      Query<Item> query = cache.query("from org.infinispan.protostream.sampledomain.Item i where i.byteVector <-> [7,6,7]~7");
      query.maxResults(3);
      List<Item> hits = query.list();
      assertThat(hits).extracting("code").containsExactly("c7", "c6", "c8");

      query = cache.query("from org.infinispan.protostream.sampledomain.Item i where i.byteVector <-> [7,6,7]~7");
      hits = query.list();
   ...
   public void withScore() {
      Query<Object[]> query = cache.query("select score(i), i from org.infinispan.protostream.sampledomain.Item i where i.byteVector <-> [7,6,7]~3");
   ...
   public void withScore_andKParam() {
      Query<Object[]> query = cache.query("select score(i), i from org.infinispan.protostream.sampledomain.Item i where i.byteVector <-> [7,6,7]~:k");
   ...
   public void withScore_andVectorParam() {
      Query<Object[]> query = cache.query("select score(i), i from org.infinispan.protostream.sampledomain.Item i where i.byteVector <-> [:v]~:k");
      query.setParameter("k", 3);
   ...
   public void withScore_andVectorCellParams() {
      Query<Object[]> query = cache.query("select score(i), i from org.infinispan.protostream.sampledomain.Item i where i.byteVector <-> [:a,:b,:c]~:k");
      query.setParameter("k", 3);
