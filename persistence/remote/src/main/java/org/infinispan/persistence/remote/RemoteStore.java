...
public class RemoteStore<K, V> implements NonBlockingStore<K, V> {
   ...
   public CompletionStage<Void> start(InitializationContext ctx) {
      ...
      // We currently don't support segmentation when not shared as we will have to support remove segments -
      // should we add this?
      if (needsStateTransfer && !configuration.shared() && configuration.segmented()) {
