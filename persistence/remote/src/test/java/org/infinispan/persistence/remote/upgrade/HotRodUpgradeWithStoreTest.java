...
public class HotRodUpgradeWithStoreTest extends AbstractInfinispanTest {
   ...
   public void setup() throws Exception {
      ...
      sourceCluster = new Cluster.Builder().setName("sourceCluster").setNumMembers(2)
            .cache().name(CACHE_NAME).configuredWith(sourceStoreBuilder)
            .build();

      targetCluster = new Cluster.Builder().setName("targetCluster").setNumMembers(2)
   ...
   private int storeSize(Cluster cluster) {
      PersistenceManager pm = extractComponent(cluster.getEmbeddedCache(CACHE_NAME), PersistenceManager.class);
