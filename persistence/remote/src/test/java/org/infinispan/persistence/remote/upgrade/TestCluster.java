...
import static org.infinispan.client.hotrod.impl.ConfigurationProperties.DEFAULT_PROTOCOL_VERSION;
      ...
      public TestCluster build() {
         ...
         for (int i = 0; i < numMembers; i++) {
            ...
            EmbeddedCacheManager clusteredCacheManager =
                  createClusteredCacheManager(gcb, hotRodCacheConfiguration(getDefaultClusteredCacheConfig(CacheMode.DIST_SYNC)));
            caches.entrySet().forEach(entry ->
                  clusteredCacheManager.defineConfiguration(entry.getKey(), hotRodCacheConfiguration(entry.getValue()).build()));
