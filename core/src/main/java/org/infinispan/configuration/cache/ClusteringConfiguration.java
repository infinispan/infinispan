...
public class ClusteringConfiguration extends ConfigurationElement<ClusteringConfiguration> {
   ...
   public boolean matches(ClusteringConfiguration other) {
      ...
      for (int i = 0; i < children.length; i++) {
         ...
         // Hash configuration does not take num-owners into account for replicated caches.
         if (ours == hashConfiguration && cacheMode().isReplicated()) {
            if (!ours.matches(theirs, ConfigurationElement.extractAttributes(ours.attributes(), HashConfiguration.NUM_OWNERS)))
               return false;
            continue;
         }
