...
import static org.infinispan.configuration.cache.RecoveryConfiguration.*;
...
public class RecoveryConfigurationBuilder extends AbstractTransportConfigurationChildBuilder implements Builder<RecoveryConfiguration> {
   ...
   RecoveryConfigurationBuilder(TransactionConfigurationBuilder builder) {
      super(builder);
      attributes = RecoveryConfiguration.attributeSet();
   }
   ...
   public void validate() {
      if (!attributes.attribute(ENABLED).asBoolean() || transaction().useSynchronization()) {
         return;
      }
