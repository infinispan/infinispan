...
public class LockingConfigurationBuilder extends AbstractConfigurationChildBuilder implements Builder<LockingConfiguration> {
   ...
   protected LockingConfigurationBuilder(ConfigurationBuilder builder) {
      super(builder);
      attributes = LockingConfiguration.attributeSet();
   }
   ...
   public void validate() {
      if (attributes.attribute(WRITE_SKEW_CHECK).asBoolean()) {
         if (attributes.attribute(ISOLATION_LEVEL).asObject(IsolationLevel.class) != IsolationLevel.REPEATABLE_READ)
      ...
      if (getBuilder().clustering().cacheMode().isClustered() && isolationLevel.asObject(IsolationLevel.class) == IsolationLevel.NONE)
         isolationLevel.set(IsolationLevel.READ_COMMITTED);

      if (isolationLevel.asObject(IsolationLevel.class) == IsolationLevel.READ_UNCOMMITTED)
         isolationLevel.set(IsolationLevel.READ_COMMITTED);

      if (isolationLevel.asObject(IsolationLevel.class) == IsolationLevel.SERIALIZABLE)
