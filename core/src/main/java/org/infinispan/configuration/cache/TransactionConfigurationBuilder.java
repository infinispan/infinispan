...
public class TransactionConfigurationBuilder extends AbstractConfigurationChildBuilder implements Builder<TransactionConfiguration> {
   ...
   TransactionConfigurationBuilder(ConfigurationBuilder builder) {
      super(builder);
      attributes = TransactionConfiguration.attributeSet();
   ...
   public void validate() {
      ...
      if (reaperWakeUpInterval.asLong()< 0)
         throw log.invalidReaperWakeUpInterval(reaperWakeUpInterval.asLong());
      if (completedTxTimeout.asLong() < 0)
         throw log.invalidCompletedTxTimeout(completedTxTimeout.asLong());
      if(attributes.attribute(TRANSACTION_PROTOCOL).asObject(TransactionProtocol.class) == TransactionProtocol.TOTAL_ORDER) {
   ...
   public TransactionConfiguration create() {
      if (attributes.attribute(USE_EAGER_LOCKING).asBoolean()) {
