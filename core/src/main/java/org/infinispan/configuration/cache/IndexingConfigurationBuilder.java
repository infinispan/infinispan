...
public class IndexingConfigurationBuilder extends AbstractConfigurationChildBuilder implements Builder<IndexingConfiguration>{
   ...
   IndexingConfigurationBuilder(ConfigurationBuilder builder) {
      super(builder);
      attributes = IndexingConfiguration.attributeSet();
   }
   ...
   public IndexingConfigurationBuilder enable() {
      ...
      if (index.asObject(Index.class) == Index.NONE)
         index.set(Index.ALL);
      return this;
   }
   ...
   public IndexingConfigurationBuilder enabled(boolean enabled) {
      Attribute<Index> index = attributes.attribute(INDEX);
      if (index.asObject(Index.class) == Index.NONE & enabled)
         index.set(Index.ALL);
      else if (!enabled)
   ...
   boolean enabled() {
      return attributes.attribute(INDEX).asObject(Index.class).isEnabled();
   }
   ...
   boolean indexLocalOnly() {
      return attributes.attribute(INDEX).asObject(Index.class).isLocalOnly();
   }
   ...
   public boolean autoConfig() {
      return attributes.attribute(AUTO_CONFIG).asBoolean();
   }
   ...
   public IndexingConfiguration create() {
      TypedProperties typedProperties = attributes.attribute(PROPERTIES).asObject(TypedProperties.class);
      if (autoConfig()) {
