...
public class AbstractStoreConfiguration implements StoreConfiguration {
   static final AttributeDefinition<Boolean> FETCH_PERSISTENT_STATE = AttributeDefinition.builder("fetchPersistentState", false).immutable().build();
   static final AttributeDefinition<Boolean> PURGE_ON_STARTUP = AttributeDefinition.builder("purgeOnStartup", false).immutable().build();
   static final AttributeDefinition<Boolean> IGNORE_MODIFICATIONS = AttributeDefinition.builder("ignoreModifications", false).immutable().build();
   static final AttributeDefinition<Boolean> PRELOAD = AttributeDefinition.builder("preload", false).immutable().build();
   static final AttributeDefinition<Boolean> SHARED = AttributeDefinition.builder("shared", false).immutable().build();
   ...
   public boolean purgeOnStartup() {
      return attributes.attribute(PURGE_ON_STARTUP).asBoolean();
   }

   @Override
   public boolean shared() {
      return attributes.attribute(SHARED).asBoolean();
   }
   ...
   public boolean fetchPersistentState() {
      return attributes.attribute(FETCH_PERSISTENT_STATE).asBoolean();
   }
   ...
   public boolean ignoreModifications() {
      return attributes.attribute(IGNORE_MODIFICATIONS).asBoolean();
   }

   @Override
   public boolean preload() {
      return attributes.attribute(PRELOAD).asBoolean();
   }

   @Override
   public Properties properties() {
      return attributes.attribute(PURGE_ON_STARTUP).asObject(Properties.class);
   }
