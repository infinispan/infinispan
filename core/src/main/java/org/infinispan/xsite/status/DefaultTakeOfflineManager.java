...
public class DefaultTakeOfflineManager implements TakeOfflineManager, XSiteResponse.XSiteResponseCompleted {
   ...
   @Inject
   public void inject(EmbeddedCacheManager cacheManager, Configuration configuration, RpcManager rpcManager, TimeService timeService, EventLogManager eventLogManager, @ComponentName(KnownComponentNames.CACHE_NAME) String cacheName) {
      this.timeService = timeService;
      this.eventLogger = eventLogManager.getEventLogger().context(cacheName).scope(rpcManager.getAddress());
      if (!offlineStatus.isEmpty()) {
         // method can be invoked multiple times
         return;
      }
      Supplier<TimeService> timeServiceSupplier = this::getTimeService;
      var localSiteName = rpcManager.getTransport().localSiteName();
