...
/**
 * It manages the keys changed in the local cluster and sends to all
 * asynchronous backup configured.
 * <p>
 * The {@code lockOwner} is the last command (or transaction) who updated the
 * key. It is used to detect conflicting local updates while sending to the
 * remote backups (sites).
 *
 * @author Pedro Ruivo
 * @since 11.0
 */
...
public interface IracManager {
    ...
    * {@link #trackUpdatedKey(int, Object, Object)}. It just returns a
    * {@link CompletionStage} to notify when the keys were sent. It is required by
    * the cross-site state transfer protocol to know when it has finish.
    *
    * @param stateList The list of {@link XSiteState}.
    * @return A {@link CompletionStage} which is completed when all the keys in
    *       {@code stateList} have been sent to the remote site.
    */
    ...
    * <p>
    * If {@code lockOwner} isn't the last one who updated the key, this method is a
    * no-op.
    *
    * @param segment   The key's segment.
    * @param key      The key.
   ...
   /**
    * Checks if the given key is expired on all other sites. If the key is expired
    * on all other sites this will return true
    *
   ...
   /**
    * Increase the count of conflicts if merge policy discard update (local value
    * wins)
    */
   void increaseNumberOfConflictLocalWins();

   /**
    * Increase the count of conflicts if merge policy applies update (remote value
    * wins)
    */
   void increaseNumberOfConflictRemoteWins();

   /**
    * Increase the count of conflicts if merge policy created a new value (merge
    * remote value with local value)
    */
   void increaseNumberOfConflictMerged();
}
