...
/**
 * Global registry that tracks the progress of state transfer across all caches.
 *
 * <p>
 * Each cache's state transfer is tracked independently via {@link CacheStateTransferTracker}, obtained through
 * {@link #forCache(String)}. The per-cache tracker coordinates the completion of three distinct phases:
 * <ul>
...
public class StateTransferTracker {
   ...
   private final Map<String, CacheStateTransferTracker> trackers = new ConcurrentHashMap<>();

   /**
    * Returns the per-cache tracker for the given cache, creating one if it does not exist.
    *
    * @param cacheName the name of the cache
    * @return the per-cache state transfer tracker
    */
   public final CacheStateTransferTracker forCache(String cacheName) {
      return trackers.computeIfAbsent(cacheName, CacheStateTransferTracker::new);
   }

   /**
    * Removes the per-cache tracker for the given cache. Called when the cache is fully stopped
    * and no longer needs tracking.
    *
    * @param cacheName the name of the cache to remove
    */
   public void remove(String cacheName) {
      trackers.remove(cacheName);
   }

   /**
    * Tracks the progress of state transfer for a single cache.
    *
    * <p>
   ...
   public static final class CacheStateTransferTracker {
      ...
      private CacheStateTransferTracker(String cacheName) {
      ...
      public CompletionStage<Void> onStateTransferCompleted(BiPredicate<CacheTopology, Throwable> listener) {
         ...
         try {
            if (log.isTraceEnabled())
               log.tracef("waiting state completion %s (consumer=%s) (provider=%s)", cacheName, consumer.isPending(), provider.isPending());
      ...
      public void cacheTopologyUpdated(CacheTopology cacheTopology) {
         ...
         if (isStableTopology) {
            ...
            try {
               ...
               consumer.complete(Integer.MIN_VALUE, null);
               provider.complete(Integer.MIN_VALUE, null);
            } finally {
               releaseLock();
            }
         }
