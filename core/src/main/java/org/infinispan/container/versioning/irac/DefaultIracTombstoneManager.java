...
public class DefaultIracTombstoneManager implements IracTombstoneManager {
   ...
   private CompletionStage<Void> performCleanup() {
      ...
      var tombstoneStage = Flowable.fromIterable(tombstoneMap.values())
            ...
            .flatMap(group -> switch (group.getKey()) {
               ...
               default -> Flowable.empty();
            }, true, ACTION_COUNT, ACTION_COUNT)
            .lastStage(null);
      if (trace) {
         tombstoneStage = tombstoneStage.whenComplete(TRACE_ROUND_COMPLETED);
      }

      var keysStage = Flowable.fromStream(iracManager.running().pendingKeys())
            .groupBy(IracManagerKeyInfo::segment)
            .filter(group -> isNotPrimaryOfSegment(group.getKey()))
            .flatMap(group -> group
                        .buffer(batchSize)
      ...
      return CompletionStages.aggregateCompletionStage()
            .dependsOn(tombstoneStage)
            .dependsOn(keysStage)
            .freeze()
   ...
   private Flowable<Void> checkKeysExistsInPrimary(List<IracManagerKeyInfo> infos) {
      ...
      return Flowable.fromCompletionStage(stage);
   }

   private boolean isNotPrimaryOfSegment(int segment) {
