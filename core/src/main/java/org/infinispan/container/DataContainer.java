...
public interface DataContainer<K, V> extends Iterable<InternalCacheEntry<K, V>> {
    ...
    /**
     * Retrieves a cache entry in the same way as {@link #get(Object)}} except that it does not
     * update or reorder any of the internal constructs. I.e., expiration does not happen, and in
     * the case of the LRU container, the entry is not moved to the end of the chain.
     * <p/>
     * This method should be used instead of {@link #get(Object)}} when called while iterating
     * through the data container using methods like {@link #keySet()} to avoid changing the
     * underlying collection's order.
     *
     * @param k key under which entry is stored
    ...
    /**
     * Puts an entry in the cache along with metadata adding information such lifespan of entry, max
     * idle time, version information...etc.
     * <p/>
    ...
    /**
     * Returns a set of keys in the container. When iterating through the container using this
     * method, clients should never call {@link #get(Object)} method but instead {@link
     * #peek(Object)}, in order to avoid changing the order of the underlying collection as a side
     * of effect of iterating through it.
     *
     ...
     * Clients of this method such as Cache.entrySet() operation implementors are free to convert
     * the set into an immutable set if needed, which is the most common use case.
     * <p/>
     * If a client needs to iterate through a mutable set of mutable cache entries, it should
     * iterate the container itself rather than iterating through the return of entrySet().
     *
     ...
     * <p/>
     * The {@code key} must be activate by invoking {@link org.infinispan.eviction.ActivationManager#onRemove(Object,
     * boolean)} or {@link org.infinispan.eviction.ActivationManager#onUpdate(Object, boolean)}
     * depending if the value returned by the {@link org.infinispan.container.DataContainer.ComputeAction}
     * is null or not respectively. <p> Note the entry provided to {@link
     * org.infinispan.container.DataContainer.ComputeAction} may be expired as these entries are not
     * filtered as many other methods do.
     *
     * @param key The key.
     * @param action The action that will compute the new value.
     * @return The {@link org.infinispan.container.entries.InternalCacheEntry} associated to the
     * key.
     */
    InternalCacheEntry<K, V> compute(K key, ComputeAction<K, V> action);

    /**
     * Executes task specified by the given action on the container key/values filtered using the
     * specified key filter.
     *
     * @param filter the filter for the container keys
     * @param action the specified action to execute on filtered key/values
     * @throws InterruptedException
     */
    void executeTask(final KeyFilter<? super K> filter,
        BiConsumer<? super K, InternalCacheEntry<K, V>> action) throws InterruptedException;

    /**
     * Executes task specified by the given action on the container key/values filtered using the
     * specified keyvalue filter.
     *
     * @param filter the filter for the container key/values
     * @param action the specified action to execute on filtered key/values
     * @throws InterruptedException
     */
    void executeTask(KeyValueFilter<? super K, ? super V> filter,
        BiConsumer<? super K, InternalCacheEntry<K, V>> action) throws InterruptedException;

    /**
     * {@inheritDoc} <p>This iterator only returns entries that are not expired, however it will not
     * remove them while doing so.</p>
     *
    ...
    interface ComputeAction<K, V> {
         ...
         * Computes the new value for the key.
         *
         * @return The new {@code InternalCacheEntry} for the key, {@code null} if the entry is to
         * be removed or {@code oldEntry} is the entry is not to be changed (i.e. not entries are
         * added, removed or touched).
         */
        InternalCacheEntry<K, V> compute(K key, InternalCacheEntry<K, V> oldEntry,
            InternalEntryFactory factory);

    }

    /**
     * Resizes the capacity of the underlying container. This is only supported if the container is
     * bounded. An {@link UnsupportedOperationException} is thrown otherwise.
     *
    ...
    /**
     * Returns the capacity of the underlying container. This is only supported if the container is
     * bounded. An {@link UnsupportedOperationException} is thrown otherwise.
     *
     * @return
     */
