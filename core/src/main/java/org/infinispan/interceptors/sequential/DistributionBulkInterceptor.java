 ...
 * interceptor produces backing collections for either method and a distributed stream for either which
 * leverages
 ...
 * @since 8.1
 */
public class DistributionBulkInterceptor<K, V> extends BaseSequentialInterceptor
   ...
   @Override
   public CompletableFuture<Object> visitEntrySetCommand(InvocationContext ctx, EntrySetCommand command)
         throws Throwable {
      ctx.onReturn((returnValue, throwable) -> {
         ...
         if (!command.hasFlag(Flag.CACHE_MODE_LOCAL)) {
            CacheSet<CacheEntry<K, V>> entrySet = (CacheSet<CacheEntry<K, V>>) returnValue;
            Cache<K, V> cacheWithFlags = CacheImpl.getCacheWithFlags(cache, command);
            if (ctx.isInTxScope()) {
               return CompletableFuture.completedFuture(
                     new TxBackingEntrySet<>(cacheWithFlags, entrySet, command,
                                             (TxInvocationContext<LocalTransaction>) ctx));
            } else {
               return CompletableFuture
                     .completedFuture(new BackingEntrySet<>(cacheWithFlags, entrySet, command));
            }
         }
         return null;
      });
   ...
   protected static class BackingEntrySet<K, V>
      ...
      private BackingEntrySet(Cache cache, CacheSet<CacheEntry<K, V>> entrySet,
      ...
      public CloseableIterator<CacheEntry<K, V>> iterator() {
         return new CloseableIteratorMapper<>(
               new RemovableCloseableIterator<>(Closeables.iterator(stream()), cache, CacheEntry::getKey),
      ...
      public CacheStream<CacheEntry<K, V>> stream() {
         ...
         CacheStream<CacheEntry<K, V>> cacheStream =
               new DistributedCacheStream<CacheEntry<K, V>>(cache.getCacheManager().getAddress(), false,
                                                            advancedCache.getDistributionManager(),
                                                            () -> entrySet.stream(),
                                                            registry.getComponent(ClusterStreamManager.class),
                                                            !command.hasFlag(Flag.SKIP_CACHE_LOAD),
                                                            cache.getCacheConfiguration().clustering()
                                                                 .stateTransfer().chunkSize(), registry
                                                                  .getComponent(Executor.class,
                                                                                ASYNC_OPERATIONS_EXECUTOR),
                                                            registry) {
                  @Override
      ...
      public CacheStream<CacheEntry<K, V>> parallelStream() {
         ...
         ComponentRegistry registry = advancedCache.getComponentRegistry();
         CacheStream<CacheEntry<K, V>> cacheStream =
               new DistributedCacheStream<>(cache.getCacheManager().getAddress(), true,
                                            advancedCache.getDistributionManager(), () -> entrySet.stream(),
                                            registry.getComponent(ClusterStreamManager.class),
                                            !command.hasFlag(Flag.SKIP_CACHE_LOAD),
                                            cache.getCacheConfiguration().clustering().stateTransfer()
                                                 .chunkSize(),
   ...
   protected static class TxBackingEntrySet<K, V> extends BackingEntrySet<K, V> {
      ...
      private TxBackingEntrySet(Cache cache, CacheSet<CacheEntry<K, V>> entrySet,
            LocalFlagAffectedCommand command, TxInvocationContext<LocalTransaction> ctx) {
      ...
      public CacheStream<CacheEntry<K, V>> stream() {
         ...
         TxClusterStreamManager<K> txManager =
               new TxClusterStreamManager<>(realManager, ctx, dm.getConsistentHash());

         CacheStream<CacheEntry<K, V>> cacheStream =
               new TxDistributedCacheStream<>(cache.getCacheManager().getAddress(), false, dm,
                                              () -> entrySet.stream(), txManager,
                                              !command.hasFlag(Flag.SKIP_CACHE_LOAD),
                                              cache.getCacheConfiguration().clustering().stateTransfer()
                                                   .chunkSize(), registry
                                                    .getComponent(Executor.class, ASYNC_OPERATIONS_EXECUTOR),
                                              registry, ctx);
         return applyTimeOut(cacheStream, cache);
      }
      ...
      public CacheStream<CacheEntry<K, V>> parallelStream() {
         ...
         CacheStream<CacheEntry<K, V>> cacheStream =
               new TxDistributedCacheStream<>(cache.getCacheManager().getAddress(), true, dm,
                                              () -> entrySet.stream(), txManager,
                                              !command.hasFlag(Flag.SKIP_CACHE_LOAD),
                                              cache.getCacheConfiguration().clustering().stateTransfer()
                                                   .chunkSize(), registry
   ...
   private static <C> CacheStream<C> applyTimeOut(CacheStream<C> stream, Cache<?, ?> cache) {
      return stream
            .timeout(cache.getCacheConfiguration().clustering().sync().replTimeout(), TimeUnit.MILLISECONDS);
   }
   ...
   @Override
   public CompletableFuture<Object> visitKeySetCommand(InvocationContext ctx, KeySetCommand command)
         throws Throwable {
      if (!command.hasFlag(Flag.CACHE_MODE_LOCAL)) {
         Set<Flag> flags =
               command.getFlags() != null ? EnumSet.copyOf(command.getFlags()) : EnumSet.noneOf(Flag.class);
         flags.add(Flag.CACHE_MODE_LOCAL);
         EntrySetCommand entrySetCommand = commandsFactory.buildEntrySetCommand(flags);
         return CompletableFuture
               .completedFuture(ctx.forkInvocation(entrySetCommand, (returnValue, throwable) -> {
                  CacheSet<CacheEntry<K, V>> localEntrySet = (CacheSet<CacheEntry<K, V>>) returnValue;
                  Cache<K, V> cacheWithFlags = CacheImpl.getCacheWithFlags(cache, command);
                  if (ctx.isInTxScope()) {
                     return CompletableFuture.completedFuture(
                           new TxBackingKeySet<>(cacheWithFlags, localEntrySet, command,
                                                 (TxInvocationContext<LocalTransaction>) ctx));
                  } else {
                     return CompletableFuture
                           .completedFuture(new BackingKeySet<>(cacheWithFlags, localEntrySet, command));
                  }
               }));
      }
      return null;
   ...
   protected static class BackingKeySet<K, V> extends AbstractCloseableIteratorCollection<K, K, V>
      ...
      public BackingKeySet(Cache<K, V> cache, CacheSet<CacheEntry<K, V>> entrySet,
            LocalFlagAffectedCommand command) {
         super(cache);
         this.entrySet = entrySet;
         this.command = command;
      ...
      public CacheStream<K> stream() {
         ...
         return new DistributedCacheStream<K>(cache.getCacheManager().getAddress(), false,
                                              ...
                                              cache.getCacheConfiguration().clustering().stateTransfer()
                                                   .chunkSize(), registry
                                                    .getComponent(Executor.class, ASYNC_OPERATIONS_EXECUTOR),
                                              registry, StreamMarshalling.entryToKeyFunction()) {
            @Override
   ...
   private static class TxBackingKeySet<K, V> extends BackingKeySet<K, V> {
      ...
      public TxBackingKeySet(Cache<K, V> cache, CacheSet<CacheEntry<K, V>> entrySet,
            LocalFlagAffectedCommand command, TxInvocationContext<LocalTransaction> ctx) {
         super(cache, entrySet, command);
         this.ctx = ctx;
      }
      ...
      public CacheStream<K> stream() {
         ...
         return new TxDistributedCacheStream<>(cache.getCacheManager().getAddress(), false, dm,
                                               () -> entrySet.stream(), txManager,
                                               !command.hasFlag(Flag.SKIP_CACHE_LOAD),
                                               cache.getCacheConfiguration().clustering().stateTransfer()
                                                    .chunkSize(), registry
                                                     .getComponent(Executor.class, ASYNC_OPERATIONS_EXECUTOR),
                                               registry, StreamMarshalling.entryToKeyFunction(), ctx);
      }
      ...
      public CacheStream<K> parallelStream() {
         ...
         return new TxDistributedCacheStream<>(cache.getCacheManager().getAddress(), true, dm,
                                               () -> entrySet.stream(), txManager,
                                               !command.hasFlag(Flag.SKIP_CACHE_LOAD),
                                               cache.getCacheConfiguration().clustering().stateTransfer()
                                                    .chunkSize(), registry
