...
public abstract class BaseDistributionInterceptor extends ClusteringInterceptor
      implements DDSequentialInterceptor {
   ...
   @Override
   public final CompletableFuture<Object> visitGetKeysInGroupCommand(InvocationContext ctx,
   ...
   @Override
   public final CompletableFuture<Object> visitClearCommand(InvocationContext ctx, ClearCommand command)
         throws Throwable {
      if (ctx.isOriginLocal() && !isLocalModeForced(command)) {
         return rpcManager
               .invokeRemotelyAsync(null, command, rpcManager.getDefaultRpcOptions(isSynchronous(command)))
   ...
   protected final CompletableFuture<InternalCacheEntry> retrieveFromRemoteSource(Object key,
      ...
      ClusteredGetCommand getCommand =
            cf.buildClusteredGetCommand(key, command.getFlags(), acquireRemoteLock, gtx);
      ...
      return handleRemoteValue(key, null, getCommand, rpcOptionsBuilder, lastTopologyId);
   }
   ...
   private CompletableFuture<InternalCacheEntry> handleRemoteValue(Object key, InternalCacheEntry value,
      ...
      if (trace) {
         log.tracef("Perform remote get for key %s. topologyId=%s, currentTopologyId=%s", key, lastTopologyId,
                    currentTopologyId);
      }
      ...
      if (lastTopologyId < currentTopologyId) {
         ...
         // We have not received a valid value from the pending CH owners either, and the topology id
         // hasn't changed
         if (trace) {
            log.tracef("No valid values found for key '%s' (topologyId=%s).", key, currentTopologyId);
         }
   ...
   protected Map<Object, InternalCacheEntry> retrieveFromRemoteSources(Set<?> requestedKeys,
         InvocationContext ctx, Set<Flag> flags) throws Throwable {
      GlobalTransaction gtx = ctx.isInTxScope() ? ((TxInvocationContext) ctx).getGlobalTransaction() : null;
      ...
      RpcOptionsBuilder rpcOptionsBuilder =
            rpcManager.getRpcOptionsBuilder(ResponseMode.SYNCHRONOUS_IGNORE_LEAVERS, DeliverOrder.NONE);
      ...
      for (Map.Entry<Address, Response> entry : responses.entrySet()) {
         updateWithValues(((ClusteredGetAllCommand) commands.get(entry.getKey())).getKeys(), entry.getValue(),
                          entries);
      }
   ...
   protected final CompletableFuture<Object> handleNonTxWriteCommand(InvocationContext ctx,
         DataWriteCommand command) throws Throwable {
      if (ctx.isInTxScope()) {
         throw new CacheException(
               "Attempted execution of non-transactional write command in a transactional invocation " +
                     "context");
      }
      ...
      if (!ctx.isOriginLocal()) {
         if (primaryOwner.equals(rpcManager.getAddress())) {
            if (!command.isSuccessful()) {
               log.tracef(
                     "Skipping the replication of the conditional command as it did not succeed on primary " +
                           "owner (%s).", command);
               return CompletableFuture.completedFuture(localResult);
            }
            ...
            try {
               rpcManager.invokeRemotely(recipients, command,
                                         determineRpcOptionsForBackupReplication(rpcManager, isSync,
                                                                                 recipients));
            } finally {
               // Switch to the retry policy, in case the primary owner changed and the write already
         ...
         if (primaryOwner.equals(rpcManager.getAddress())) {
            if (!command.isSuccessful()) {
               log.tracef(
                     "Skipping the replication of the command as it did not succeed on primary owner (%s).",
                     command);
               return CompletableFuture.completedFuture(localResult);
            }
            List<Address> recipients = cdl.getOwners(command.getKey());
            log.tracef("I'm the primary owner, sending the command to all the backups (%s) in order to be " +
                             "applied.", recipients);
            ...
            log.tracef(
                  "I'm not the primary owner, so sending the command to the primary owner(%s) in order to " +
                        "be forwarded", primaryOwner);
            boolean isSyncForwarding = isSync || command.isReturnValueExpected();

            Map<Address, Response> addressResponseMap;
            try {
               addressResponseMap = rpcManager
                     .invokeRemotely(Collections.singletonList(primaryOwner), command,
               ...
               if (ce instanceof OutdatedTopologyException) {
                  // If the primary owner throws an OutdatedTopologyException, it must be because the
                  // command succeeded there
                  if (trace)
                     log.tracef("Changing the value matching policy from %s to %s (original value was %s)",
                                command.getValueMatcher(), valueMatcher.matcherForRetry(), valueMatcher);
                  command.setValueMatcher(valueMatcher.matcherForRetry());
               }
               throw e;
            } catch (SuspectException e) {
               // If the primary owner became suspected, we don't know if it was able to replicate it's
               // data properly
               // to all backup owners and notify all listeners, thus we need to retry with new matcher in
               // case if
               // it had updated the backup owners
               if (trace)
                  log.tracef("Primary owner suspected - Changing the value matching policy from %s to %s " +
   ...
   private RpcOptions determineRpcOptionsForBackupReplication(RpcManager rpc, boolean isSync,
   ...
   private Object getResponseFromPrimaryOwner(Address primaryOwner,
         Map<Address, Response> addressResponseMap) {
      ...
      Throwable cause = fromPrimaryOwner instanceof ExceptionResponse ?
                        ((ExceptionResponse) fromPrimaryOwner).getException() : null;
      throw new CacheException("Got unsuccessful response from primary owner: " + fromPrimaryOwner, cause);
   }

   @Override
   public CompletableFuture<Object> visitGetAllCommand(InvocationContext ctx, GetAllCommand command)
         throws Throwable {
      ...
      if (ctx.isOriginLocal()) {
         ...
         if (trace) {
            log.tracef("Command topology id is %d, current topology id is %d", commandTopologyId,
                       currentTopologyId);
         }
         if (topologyChanged) {
            throw new OutdatedTopologyException(
               ...
               if (!isValueAvailableLocally(ch, key)) {
                  ...
                  if (trace) {
                     log.tracef("Not doing a remote get for missing key %s since entry is " +
                                      "mapped to current node (%s). Owners are %s", toStr(key),
                                rpcManager.getAddress(), ch.locateOwners(key));
                  }
         ...
         if (!requestedKeys.isEmpty()) {
            ...
            Map<Object, InternalCacheEntry> justRetrieved =
                  retrieveFromRemoteSources(requestedKeys, ctx, command.getFlags());
   ...
   @Override
   public CompletableFuture<Object> visitReadOnlyManyCommand(InvocationContext ctx,
         ReadOnlyManyCommand command) throws Throwable {
   ...
   protected abstract CompletableFuture<Object> remoteGetBeforeWrite(InvocationContext ctx,
   ...
   protected boolean readNeedsRemoteValue(InvocationContext ctx, AbstractDataCommand command) {
      ...
      if (!shouldFetchFromRemote && trace) {
         log.tracef(
               "Not doing a remote get for key %s since entry is mapped to current node (%s) or is in L1. " +
                     "Owners are %s", toStr(key), rpcManager.getAddress(), ch.locateOwners(key));
      }
