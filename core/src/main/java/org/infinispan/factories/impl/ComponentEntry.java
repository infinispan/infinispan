...
final class ComponentEntry {
   ...
   private static final class ComponentTiming {
      ...
      public long duration(TimeUnit unit) {
         ...
         if (end == NOT_RECORDED) return NOT_RECORDED;

         return unit.convert(end - instantiating, TimeUnit.NANOSECONDS);
      }

      /**
       * Duration in milliseconds of the preceding phase relative to the given state.
       * <p>
       * For example, given an INSTANTIATED phase, it returns the time spent in INSTANTIATING.
       * </p>
       *
       * @param unit The time unit to output the duration.
       * @return -1 if insufficient data. Otherwise, the time spent from instantiating up to the given phase in the requested
       *         time unit.
       */
      public long phaseDuration(TimeUnit unit, BasicComponentRegistryImpl.WrapperState phase) {
         long start = phaseStartTime(phase);
         long end = resolvedTime(phase);
         if (start == NOT_RECORDED || end == NOT_RECORDED) return NOT_RECORDED;
      ...
      /**
       * Resolve the timestamp for a given state, falling back to the previous phase when that state was skipped.
       */
      private long resolvedTime(BasicComponentRegistryImpl.WrapperState state) {
         return switch (state) {
            case INSTANTIATING -> instantiating;
            case INSTANTIATED -> instantiated != NOT_RECORDED ? instantiated : resolvedTime(BasicComponentRegistryImpl.WrapperState.INSTANTIATING);
            case WIRING -> wiring != NOT_RECORDED ? wiring : resolvedTime(BasicComponentRegistryImpl.WrapperState.INSTANTIATED);
            case WIRED -> wired != NOT_RECORDED ? wired : resolvedTime(BasicComponentRegistryImpl.WrapperState.WIRING);
            case STARTING -> starting != NOT_RECORDED ? starting : resolvedTime(BasicComponentRegistryImpl.WrapperState.WIRED);
            case STARTED -> started != NOT_RECORDED ? started : resolvedTime(BasicComponentRegistryImpl.WrapperState.STARTING);
            default -> NOT_RECORDED;
         };
      }

      /**
       * Returns the start of the phase that the given completed state ends. INSTANTIATED ends the INSTANTIATING phase,
       * so return instantiating time.
       */
      private long phaseStartTime(BasicComponentRegistryImpl.WrapperState completedState) {
         return switch (completedState) {
            case INSTANTIATED -> resolvedTime(BasicComponentRegistryImpl.WrapperState.INSTANTIATING);
            case WIRING -> resolvedTime(BasicComponentRegistryImpl.WrapperState.INSTANTIATED);
            case WIRED -> resolvedTime(BasicComponentRegistryImpl.WrapperState.WIRING);
            case STARTING -> resolvedTime(BasicComponentRegistryImpl.WrapperState.WIRED);
            case STARTED -> resolvedTime(BasicComponentRegistryImpl.WrapperState.STARTING);
            default -> NOT_RECORDED;
         };
      }
   }
