            ...
            synchronized (this) {
               if (asyncTransportExecutor == null) {
                  asyncTransportExecutor = createExecutorService(
                        globalConfiguration.transport().threadPool(),
      ...
      switch (type) {
         case SCHEDULED:
            return new ThreadPoolExecutorFactory() {
               @Override
               public <T extends ExecutorService> T createExecutor(ThreadFactory factory) {
                  return (T) Executors.newSingleThreadScheduledExecutor(factory);
               }
            };
         default:
            return new ThreadPoolExecutorFactory() {
               ...
               public ThreadPoolExecutor createExecutor(ThreadFactory threadFactory) {
                  ...
                  BlockingQueue<Runnable> queue = defaultQueueSize == 0 ?
                        new SynchronousQueue<Runnable>() :
                        new LinkedBlockingQueue<Runnable>(100000);

                  int maxThreads = KnownComponentNames.getDefaultThreads(componentName);
                  int coreThreads = defaultQueueSize == 0 ? 1 : maxThreads;
                  long keepAlive = 60000;
