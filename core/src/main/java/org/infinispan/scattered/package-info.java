 ...
 * in metadata. These timestamps also determine order of writes; we don't have to use locking anymore.
 * <p>
 * Pros:
 * <ul>
 *    <li>faster writes
 ...
 * (as tombstone will be eventually removed, per-entry version would be lost).
 * <p>
 * Unlike other cache modes, entry commit does not happen in {@link org.infinispan.interceptors.impl.EntryWrappingInterceptor}
 * but before replication to backup in {@link org.infinispan.interceptors.distribution.ScatteringInterceptor}
 ...
 * {@link org.infinispan.interceptors.impl.BaseEntryWrappingInterceptor} and committing it. Therefore, for command
 * that {@link org.infinispan.commands.write.WriteCommand#readsExistingValues()} the version seen before modification
 * is checked against actual data-container value and if it does not match,
 ...
 *    <li>Primary increments timestamp for segment
 *    <li>Primary commits entry
 ...
 * <h3>Invalidations</h3>
 *
 * It would be inefficient to send invalidations (key + timestamp) one-by-one, so these are be merged
 ...
 *
 * Scattered cache does not need to differentiate between {@link org.infinispan.topology.CacheTopology#currentCH},
 * {@link org.infinispan.topology.CacheTopology#pendingCH} and {@link org.infinispan.topology.CacheTopology#unionCH}
 * and it always uses the write-consistent hash, which should have the (exactly one) primary owner defined at all times.
 * <p>
 * The segment state and ability to read/write on a node is tracked in {@link org.infinispan.scattered.ScatteredVersionManager}.
 ...
 *    <li>BLOCKED: node has just become an owner, and does not have any information about the segment
 ...
 * <ul>
 *    <li>{@code GET_MAX_VERSIONS} that asks for highest versions for given segment (based on both the current
 *        information in {@link org.infinispan.scattered.ScatteredVersionManager} and data in data container and
 *        cache stores, which need to be iterated through.
 *    <li>{@code START_KEYS_TRANSFER} that is very similar to {@code START_STATE_TRANSFER} but moves only keys.
 * </ul>
 * <p>
 * Currently the state transfer reuses a lot from other caches, and it's still uses 3 topologies:
 * <ol>
 *    <li>CH_UPDATE just removing the dead member
 *    <li>REBALANCE starts shuffling entries around
 *    <li>CH_UPDATE with final topology
 * </ol>
 * Single topology change should be sufficient.
 * <p>
 ...
 *    <li>Send {@link org.infinispan.commands.write.InvalidateVersionsCommand}s with all keys in this segment
 * </ol>
 ...
 *    <li>Retrieve values from nodes with highest version
 * </ol>
 * <strong>TODO: </strong>Send invalidation when overwriting key with higher version.
 *
 ...
 * <p>
 * <strong>TODO: </strong> if we load entry from cache loader, we use this version for the check during commit
 *                        in {@link org.infinispan.interceptors.distribution.ScatteringInterceptor}.
 *                        However if the data-container is empty, it might fail (not sure about this) - checkme.
 *
 ...
 *
 * <h3>Iteration</h3>
 * I am not sure how this works along the state transfer :-/ See
 * {@link org.infinispan.interceptors.impl.PrefetchInterceptor.BackingIterator}
 */
