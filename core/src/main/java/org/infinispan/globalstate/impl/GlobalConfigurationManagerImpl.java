...
public class GlobalConfigurationManagerImpl implements GlobalConfigurationManager {
   ...
   private void ensureClusterCompatibility(String name, CacheState state, Map<String, Configuration> configs) {
      ...
      if (persisted != null) {
         ...
         if (areConfigurationsIncompatible(persisted, configuration))
   ...
   private void ensurePersistenceCompatibility(String name, Configuration existing, Configuration other) {
      if (areConfigurationsIncompatible(existing, other))
         throw CONFIG.incompatiblePersistedConfiguration(name, other, existing);
   }

   private boolean areConfigurationsIncompatible(Configuration one, Configuration another) {
      if (one != null && !one.matches(another)) {
         ...
         Configuration existingCopy = new ConfigurationBuilder().read(one).build(false);
         Configuration otherCopy = new ConfigurationBuilder().read(another).build(false);
         ...
         return !existingCopy.matches(otherCopy);
      }
      return false;
   }
   ...
   public CompletionStage<Void> createTemplate(String name, Configuration configuration, EnumSet<CacheContainerAdmin.AdminFlag> flags) {
      ...
      return cache.containsKeyAsync(key).thenCompose(exists -> {
         ...
         return cache.putAsync(key, new CacheState(null, configuration.toStringConfiguration(name, MediaType.APPLICATION_XML, true, targetSchemaVersion()), flags));
      }).thenApply(v -> null);
   }
   ...
   public CompletionStage<Configuration> getOrCreateTemplate(String name, Configuration configuration, EnumSet<CacheContainerAdmin.AdminFlag> flags) {
      ...
      try {
         final CacheState state = new CacheState(null, configuration.toStringConfiguration(name, MediaType.APPLICATION_XML, true, targetSchemaVersion()), flags);
   ...
   private CompletionStage<Object> createCacheInternal(String cacheName, String template, Configuration configuration, EnumSet<CacheContainerAdmin.AdminFlag> flags) {
      ...
      try {
         ...
         state = new CacheState(template, replicated.toStringConfiguration(cacheName, MediaType.APPLICATION_XML, true, targetSchemaVersion()), flags);
      } catch (Exception e) {
