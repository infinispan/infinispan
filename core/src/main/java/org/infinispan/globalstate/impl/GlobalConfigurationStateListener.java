package org.infinispan.globalstate.impl;

import org.infinispan.globalstate.ScopedState;
import org.infinispan.notifications.Listener;
import org.infinispan.notifications.cachelistener.annotation.CacheEntryCreated;
import org.infinispan.notifications.cachelistener.annotation.CacheEntryRemoved;
import org.infinispan.notifications.cachelistener.event.CacheEntryCreatedEvent;
import org.infinispan.notifications.cachelistener.event.CacheEntryRemovedEvent;

/**
 * Listens to events on the global state cache and manages cache configuration creation / removal accordingly
 *
 * @author Tristan Tarrant
 * @since 9.2
 */
@Listener(observation = Listener.Observation.POST, clustered = true, includeCurrentState = true)
public class GlobalConfigurationStateListener {
   private final GlobalConfigurationManagerImpl gcm;

   GlobalConfigurationStateListener(GlobalConfigurationManagerImpl gcm) {
      this.gcm = gcm;
   }

   @CacheEntryCreated
   public void createCache(CacheEntryCreatedEvent<ScopedState, String> event) {
      String cacheName = event.getKey().getName();
      String cacheConfiguration = event.getCache().get(event.getKey());
      gcm.localCreateCache(cacheName, cacheConfiguration);
   }

   @CacheEntryRemoved
   public void removeCache(CacheEntryRemovedEvent<ScopedState, String> event) {
      String cacheName = event.getKey().getName();
      gcm.localRemoveCache(cacheName);
   }
}
