...
/**
 * This component really is just a wrapper around a {@link org.infinispan.remoting.transport.Transport}
 * implementation, and is used to set up the transport and provide lifecycle and dependency hooks
 * into external transport implementations.
 *
 ...
 * @since 4.0
 */
@MBean(objectName = "RpcManager", description = "Manages all remote calls to " +
    "remote cache instances in the cluster.")
public class RpcManagerImpl implements RpcManager, JmxStatisticsExposer {
    ...
    @Override
    public CompletableFuture<Map<Address, Response>> invokeRemotelyAsync(
        Collection<Address> recipients,
        ReplicableCommand rpc, RpcOptions options) {
        if (trace)
            log.tracef("%s invoking %s to recipient list %s with options %s", t.getAddress(), rpc, recipients, options);
        ...
        return invocation.handle((responseMap, throwable) -> {
            ...
            if (throwable == null) {
                ...
                if (trace)
                    log.tracef("Response(s) to %s is %s", rpc, responseMap);
                return responseMap;
            } else {
                if (statisticsEnabled)
                    replicationFailures.incrementAndGet();
    ...
    @Override
    public Map<Address, Response> invokeRemotely(Map<Address, ReplicableCommand> rpcs,
        RpcOptions options) {
        if (trace)
            log.tracef("%s invoking %s with options %s", t.getAddress(), rpcs, options);
        ...
        if (statisticsEnabled)
            startTimeNanos = timeService.time();
        try {
            ...
            if (statisticsEnabled)
                replicationCount.incrementAndGet();
            if (trace)
                log.tracef("Response(s) to %s is %s", rpcs, result);
            return result;
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new CacheException("Thread interrupted while invoking RPC", e);
    ...
    @Override
    public void sendToMany(Collection<Address> destinations, ReplicableCommand command,
    ...
    public void setStatisticsEnabled(
        @Parameter(name = "enabled", description = "Whether statistics should be enabled or disabled (true/false)") boolean statisticsEnabled) {
    ...
    public RpcOptionsBuilder getRpcOptionsBuilder(ResponseMode responseMode) {
        return getRpcOptionsBuilder(responseMode, responseMode.isSynchronous() ?
            DeliverOrder.NONE : DeliverOrder.PER_SENDER);
    }

    @Override
    public RpcOptionsBuilder getRpcOptionsBuilder(ResponseMode responseMode,
        DeliverOrder deliverOrder) {
        return new RpcOptionsBuilder(configuration.clustering().remoteTimeout(),
            TimeUnit.MILLISECONDS, responseMode, deliverOrder);
    }
