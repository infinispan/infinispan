...
abstract class AdaptiveTimeoutVegas extends AdaptiveTimeoutPad3 {
   ...
   // Bits 0-31  (32 bits): baseRttKibiNs in kibi-nanoseconds (units of 1024, max ~1.2 hours)
...
public final class AdaptiveTimeout extends AdaptiveTimeoutVegas {
   ...
   public void recordTimeout() {
      ...
      // Reactive Update: Bucket has drained.
      // updateLimit will instantly halve the concurrency limit.
      long srtt = srttOf(next);
      updateLimit(srtt, srtt, newBucket, true);
   }
   ...
   public long adjustTimeout(long configuredTimeoutNs) {
      ...
      if (currentInFlight > 0 && srttKibiNs > 0) {
         // Calculate the accumulated time debt of pending operations in kibi-nanoseconds to avoid overflows.
         ...
         if (timeDebtKibiNs >= configuredTimeoutKibiNs) {
            ...
            // Safe: timeDebtKibiNs < configuredTimeoutKibiNs, diffKibiNs * timeDebtKibiNs should remain below Long.MAX_VALUE.
   ...
   private static long calculateBoundedFloorNs(long srttKibiNs, long rttvarKibiNs, long configuredTimeoutKibiNs) {
      ...
      // Convert back from kibi-ns to ns
      long boundedFloorNs = boundedFloorKibiNs << 10;


      // Guarantee the result is > 0 when a positive timeout was configured
      // Apply minimum floor of 1ms to avoid a timeout of zero.
      if (configuredTimeoutKibiNs > 0 && boundedFloorNs == 0)
         boundedFloorNs = 1_000_000L;

      return boundedFloorNs;
   }
