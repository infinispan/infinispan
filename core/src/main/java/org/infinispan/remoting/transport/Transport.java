...
/**
 * An interface that provides a communication link with remote caches.  Also allows remote caches to
 * invoke commands on this cache instance.
 *
...
public interface Transport extends Lifecycle {
     ...
     *
     * @param recipients a list of Addresses to invoke the call on.  If this is null, the call is
     ...
     * @param responseFilter a response filter with which to filter out failed/unwanted/invalid
     * responses.
     * @param deliverOrder the {@link org.infinispan.remoting.inboundhandler.DeliverOrder}.
     * @param anycast used when {@param totalOrder} is {@code true}, it means that it must use TOA
     * instead of TOB.
     ...
     */
    void sendTo(Address destination, ReplicableCommand rpcCommand,
     ...
     * @throws Exception if there was problem sending the request.
     */
    void sendToMany(Collection<Address> destinations, ReplicableCommand rpcCommand,
        DeliverOrder deliverOrder) throws Exception;

    /**
     * @deprecated Use {@link #invokeRemotely(Map, ResponseMode, long, ResponseFilter, DeliverOrder,
     * boolean)} instead
     */
    @Deprecated
    Map<Address, Response> invokeRemotely(Map<Address, ReplicableCommand> rpcCommands,
        ResponseMode mode, long timeout,
    ...
    BackupResponse backupRemotely(Collection<XSiteBackup> backups,
        XSiteReplicateCommand rpcCommand) throws Exception;
    ...
    /**
     * Retrieves the current cache instance's physical network addresses. Some implementations might
     * differentiate between logical and physical addresses in which case, this method allows
     * clients to query the physical ones associated with the logical address. Implementations where
     * logical and physical address are the same will simply return a single entry List that
     * contains the same Address as {@link #getAddress()}.
     *
    ...
    /**
     * check if the transport has configured with total order deliver properties (has the sequencer
     * in JGroups protocol stack.
     */
     ...
     * replication working.
     *
     * @return set containing the connected sites, or null if no cross site replication has been
     * enabled.
     */
