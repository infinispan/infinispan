...
public class CommandAwareRpcDispatcher extends MessageDispatcher {
     ...
     */
    public CompletableFuture<Responses> invokeRemoteCommands(List<Address> recipients,
    ...
    public SingleResponseFuture invokeRemoteCommand(Address recipient, ReplicableCommand command,
        ResponseMode mode, long timeout, DeliverOrder deliverOrder) {
    ...
    public void handle(Message req, org.jgroups.blocks.Response response) throws Exception {
        if (isValid(req)) {
            ...
            try {
                cmd = (ReplicableCommand) ispnMarshaller.objectFromByteBuffer(req.getRawBuffer(),
    ...
    private static void encodeDeliverMode(RequestOptions request, DeliverOrder deliverOrder) {
        switch (deliverOrder) {
            ...
            case PER_SENDER:
                ...
                break;
            case NONE:
                request.setFlags((short) (Message.Flag.OOB.value() |
    ...
    private void reply(org.jgroups.blocks.Response response, Object retVal,
        ReplicableCommand command, Message req) {
        if (response != null) {
            if (trace)
    ...
    static RequestOptions constructRequestOptions(ResponseMode mode, boolean rsvp,
    ...
    protected SingleResponseFuture processSingleCall(ReplicableCommand command, long timeout,
        Address destination, ResponseMode mode, DeliverOrder deliverOrder) throws Exception {
        if (trace)
            log.tracef("Replication task sending %s to single recipient %s with response mode %s", command,
                destination, mode);
        ...
        if (timeout > 0 && !retval.isDone()) {
            ScheduledFuture<?> timeoutFuture = timeoutExecutor.schedule(retval, timeout,
    ...
    private void processCallsStaggered(ReplicableCommand command, RspFilter filter,
        List<Address> dests, ResponseMode mode, DeliverOrder deliverOrder,
        ...
        CompletableFuture<Rsp<Response>> subFuture = processSingleCall(command, -1, dest, mode,
            deliverOrder);
        if (subFuture != null) {
            subFuture.whenComplete((rsp, throwable) -> {
                ...
                if (rsp.wasReceived() && (filter == null || filter.isAcceptable(rsp.hasException() ? rsp.getException() : rsp.getValue(), dest))) {
                    ...
                    if (trace)
                        log.tracef("Got acceptable response: " + rsps);
                    ...
                    if (!rsps.isMissingResponses()) {
                        ...
                        if (trace)
                            log.tracef("No missing responses: " + rsps);
                        theFuture.complete(rsps);
                    } else {
            ...
            if (!subFuture.isDone()) {
                ...
                ScheduledFuture<?> timeoutTask = timeoutExecutor.schedule(
                    () -> staggeredProcessNext(command, filter, dests, mode, deliverOrder,
                        theFuture, destIndex, deadline,
    ...
    private void staggeredProcessNext(ReplicableCommand command, RspFilter filter,
        List<Address> dests, ResponseMode mode, DeliverOrder deliverOrder,
        ...
        if (timeService.isTimeExpired(deadline)) {
            rsps.setTimedOut();
            if (trace)
                log.tracef("All requests timed out: " + rsps);
            theFuture.complete(rsps);
            return;
        }
        try {
            processCallsStaggered(command, filter, dests, mode, deliverOrder, theFuture,
    ...
    private CompletableFuture<Responses> processCalls(ReplicableCommand command, boolean broadcast,
        long timeout,
