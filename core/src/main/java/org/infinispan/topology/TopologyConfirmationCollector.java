...
class TopologyConfirmationCollector {
   ...
   public TopologyConfirmationCollector(String cacheName, int topologyId, Collection<Address> members, Runnable whenCompleted) {
    ...
    */
   public void confirmTopologyUpdate(Address node, int receivedTopologyId) {
      synchronized (this) {
         if (topologyId > receivedTopologyId) {
            throw new CacheException(String.format("Received invalid topology confirmation from %s " +
