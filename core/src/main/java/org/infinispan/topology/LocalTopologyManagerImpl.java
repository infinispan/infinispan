   ...
   public boolean isTotalOrderCache(String cacheName) {
      if (!running) {
         if (log.isTraceEnabled()) {
            log.tracef("isTotalOrderCache(%s) returning false because the local cache manager is not running",
                       cacheName);
         }
         return false;
      }
      ...
      if (cacheStatus == null) {
         if (log.isTraceEnabled()) {
            log.tracef("isTotalOrderCache(%s) returning false because the cache doesn't exist locally",
                       cacheName);
         }
         return false;
      }
