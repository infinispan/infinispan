...
public class ClusterCacheStatus implements AvailabilityStrategyContext {
   ...
   private TopologyConfirmationCollector topologyConfirmationCollector;
    ...
    */
   @GuardedBy("this")
   private void initTopologyConfirmationCollector(CacheTopology newTopology, Runnable whenCompleted) {
   ...
   public synchronized void doTopologyConfirm(Address member, int receivedTopologyId) throws Exception {
      if (topologyConfirmationCollector == null) {
      ...
      topologyConfirmationCollector.confirmTopologyUpdate(member, receivedTopologyId);
   }
   ...
   private void updateMembers() {
      if (topologyConfirmationCollector != null) {
         ...
         topologyConfirmationCollector.updateMembers(currentTopology.getMembers());
      }
   }
   ...
   private void endRebalance() {
      ...
      synchronized (this) {
         ...
         topologyConfirmationCollector = new TopologyConfirmationCollector(cacheName, currentTopologyId + 1,
   ...
   private void endReadAllPhase() {
      ...
      synchronized (this) {
         ...
         topologyConfirmationCollector = new TopologyConfirmationCollector(cacheName, currentTopology.getTopologyId() + 1,
   ...
   public synchronized void updateCurrentTopology(List<Address> newMembers) {
      ...
      CacheTopology newTopology = new CacheTopology(topologyId + 1, rebalanceId, newCurrentCH, null,
            CacheTopology.Phase.STABLE, actualMembers, persistentUUIDManager.mapAddresses(actualMembers));
      setCurrentTopology(newTopology);

      if (topologyConfirmationCollector != null) {
         // The node that will cancel the state transfer because of another topology update won't send topology confirm
         log.debugf("Cancelling topology confirmation %s because of another topology update", topologyConfirmationCollector);
         topologyConfirmationCollector = null;
      }
   ...
   public void startQueuedRebalance() {
      synchronized (this) {
         ...
         assert topologyConfirmationCollector == null;
         topologyConfirmationCollector = new TopologyConfirmationCollector(cacheName, newTopology.getTopologyId(),
