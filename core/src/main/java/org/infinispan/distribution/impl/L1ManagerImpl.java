...
public class L1ManagerImpl implements L1Manager, RemoteValueRetrievedListener {
    ...
    @Inject
    public void init(Configuration configuration, RpcManager rpcManager,
    ...
    @Stop(priority = 3)
    public void stop() {
        if (scheduledRequestorsCleanupTask != null)
            scheduledRequestorsCleanupTask.cancel(true);
    }
    ...
    @Override
    public Future<?> flushCache(Collection<Object> keys, Address origin,
    ...
    private Collection<Address> buildInvalidationAddressList(Collection<Object> keys,
        Address origin, boolean assumeOriginKeptEntryInL1) {
    ...
    private boolean isUseMulticast(int nodes) {
        ...
        if (!rpcManager.getTransport().isMulticastCapable())
            return false;
