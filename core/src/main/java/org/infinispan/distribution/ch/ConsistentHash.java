...
public interface ConsistentHash {
    ...
    /**
     * @return The configured number of owners for each key. Note that {code @getOwners(key)} may return a different
     * number of owners.
     */
    int getNumOwners();

    /**
     * @deprecated Since 8.2, the {@code Hash} is optional - replaced in the configuration by the {@code KeyPartitioner}
     */
    @Deprecated
    ...
    /**
     * @return The actual number of hash space segments. Note that it may not be the same as the number of segments
     ...
     *
     * @param key key to locate
     * @return An unmodifiable list of addresses where the key resides. Will never be {@code null}, and it will always
    ...
    /**
     * @return All the nodes that own a given hash space segment, first address is the primary owner. The returned list
     * is unmodifiable.
     */
    List<Address> locateOwnersForSegment(int segmentId);

    /**
     * @return The primary owner of a given hash space segment. This is equivalent to {@code
    ...
    /**
     * @return {@code true} if every member owns every segment. This allows callers to skip computing the segment of a
     * key in some cases.
     */
