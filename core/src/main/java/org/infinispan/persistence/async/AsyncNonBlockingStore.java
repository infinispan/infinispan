...
public class AsyncNonBlockingStore<K, V> extends DelegatingNonBlockingStore<K, V> implements Consumer<Flowable<AsyncNonBlockingStore.Modification>> {
   ...
   public void accept(Flowable<Modification> modificationFlowable) {
      modificationFlowable.subscribe(modification -> {
         ...
         synchronized (this) {
            assert modificationsToReplicate == null || modificationsToReplicate.isEmpty();
            modificationsToReplicate = modificationMap;
            modificationMap = newMap;
            clearToReplicate = hasClear;
            hasClear = false;
         }
         ...
         if (clearToReplicate) {
            if (trace) {
               log.tracef("Sending clear to underlying store for id %s", System.identityHashCode(modificationsToReplicate));
            }
            asyncBatchStage = retry(actual::clear, persistenceConfiguration.connectionAttempts()).whenComplete((ignore, t) -> {
               synchronized (this) {
                  clearToReplicate = false;
               }
            });
         } else {
         ...
         asyncBatchStage.whenComplete((ignore, t) -> {
            if (trace) {
               log.trace("Async operations completed");
            }
            boolean submitNewBatch;
            CompletableFuture<Void> future;
            synchronized (this) {
               submitNewBatch = !modificationMap.isEmpty() || hasClear;
               future = batchFuture;
   ...
   private CompletionStage<Void> replicateModifications(Map<Object, Modification> modifications) {
      // User a connected flowable, so we don't have to iterate over the modifications twice
   ...
   private Map.Entry<Boolean, Map<Object, Modification>> flattenModificationMaps() {
      ...
      synchronized (this) {
         modificationCopy = new HashMap<>(modificationMap);
         if (hasClear) {
            return new AbstractMap.SimpleImmutableEntry<>(Boolean.TRUE, modificationCopy);
         }
         modificationsToReplicate = this.modificationsToReplicate;
         clearToReplicate = this.clearToReplicate;
      }
   ...
   private CompletionStage<MarshallableEntry<K, V>> getStageFromPending(Object key) {
      ...
      synchronized (this) {
         ...
         Modification modification = modificationMap.get(wrappedKey);
   ...
   private synchronized CompletionStage<Void> asyncOrThrottledStage() {
      if (modificationMap.size() > modificationQueueSize) {
         if (trace) {
   ...
   public CompletionStage<Void> removeSegments(IntSet segments) {
      assertNotStopped();
      synchronized (this) {
         modificationMap.values().removeIf(modification -> segments.contains(modification.getSegment()));
      }
   ...
   public CompletionStage<Boolean> isAvailable() {
      ...
      return superAvailableStage.thenApply(delegateAvailable -> {
         ...
         int queueSize;
         synchronized (this) {
            isReplicating = (modificationsToReplicate != null && !modificationsToReplicate.isEmpty()) || clearToReplicate;
            queueSize = modificationMap.size();
   ...
   private static class RemoveModification implements AsyncNonBlockingStore.Modification {
      ...
      public <K, V> void apply(AsyncNonBlockingStore<K, V> store) {
         if (trace) {
            log.tracef("Adding remove modification %s to %s", this, System.identityHashCode(store.modificationMap));
         }
         store.modificationMap.put(wrapKeyIfNeeded(key), this);
      }
   ...
   private static class PutModification implements AsyncNonBlockingStore.Modification {
      ...
      public <K, V> void apply(AsyncNonBlockingStore<K, V> store) {
         if (trace) {
            log.tracef("Adding put modification %s to %s", this, System.identityHashCode(store.modificationMap));
         }
         store.modificationMap.put(wrapKeyIfNeeded(entry.getKey()), this);
      }
   ...
   private static class ClearModification implements AsyncNonBlockingStore.Modification {
      ...
      public static ClearModification INSTANCE = new ClearModification();

      @Override
      public <K, V> void apply(AsyncNonBlockingStore<K, V> store) {
         store.modificationMap.clear();
         store.hasClear = true;
      }
