...
public interface NonBlockingStore<K, V> {
   ...
   enum Characteristic {
       ...
       * {@link #entryPublisher(IntSet, Predicate)}  and {@link #size(IntSet)} methods.
       */
      BULK_READ,
      /**
    ...
    * @return
    */
   CompletionStage<MarshallableEntry<K, V>> loadEntry(int segment, Object key);
    ...
    * @return
    */
   default Publisher<MarshallableEntry<K, V>> entryPublisher(IntSet segments, Predicate<? super K> filter) {
    ...
    * </table>
    * <p>
    * A default implementation could be done with the following code if
    * {@link #entryPublisher(IntSet, Predicate)} is already implemented.
    * <pre>{@code
    * return Flowable.fromPublisher(entryPublisher(segments, filter))
    *     .map(MarshallableEntry::getKey);}
    * </pre>
    * @param segments A set of segments to filter keys by. If a null is provided, consider it as all segments.
   ...
   /**
    * Some stores may not want to perform a write operation if a command has certain flags
    * @implSpec
    * The default implementation returns true
    * @param commandFlags the flags attributed to the command when performing the write
    * @return whether the write should occur
    */
   default boolean shouldWrite(long commandFlags) {
      return true;
   }
