 ...
 * org.infinispan.configuration.cache.Configuration} or a path or URL to a configuration XML file: see {@link
 * org.infinispan.manager.DefaultCacheManager}.
 * <p/>
 * Lifecycle - <tt>EmbeddedCacheManager</tt>s have a lifecycle (it implements {@link Lifecycle}) and the default
 * constructors also call {@link #start()}.  Overloaded versions of the constructors are available, that do not start
 * the <tt>CacheManager</tt>, although it must be kept in mind that <tt>CacheManager</tt>s need to be started before
 * they can be used to create <tt>Cache</tt> instances.
 * <p/>
 * Once constructed, <tt>EmbeddedCacheManager</tt>s should be made available to any component that requires a
 * <tt>Cache</tt>, via <a href="http://en.wikipedia.org/wiki/Java_Naming_and_Directory_Interface">JNDI</a> or via some
 * other mechanism such as an <a href="http://en.wikipedia.org/wiki/Dependency_injection">dependency injection</a>
 * framework.
 * <p/>
 *
...
public interface EmbeddedCacheManager extends CacheContainer, Listenable {
     ...
     * <p/>
     * Unlike previous versions of Infinispan, this method does not build on an existing configuration (default or
     * named). If you want this behavior, then use {@link ConfigurationBuilder#read(org.infinispan.configuration.cache.Configuration)}.
     * <p/>
    ...
    /**
     * Removes a configuration from the set of defined configurations. If the configuration is currently in use by one
     * of the caches, an {@link IllegalStateException} is thrown. If the named configuration does not exist, nothing
     * happens
     *
     ...
     *
     * @return an immutable set of non-default named caches registered or created with this cache manager.
     */
     ...
     *
     * @param cacheName
     * @return <tt>true</tt> if the cache with the given name has not yet been started, or if it was started, whether
     ...
     * @param createIfAbsent if <tt>false</tt>, the named cache will not be retrieved if it hasn't been retrieved
     * previously or if it's been removed. If <tt>true</tt>, this methods works just like {@link #getCache(String)}
     * @return null if no named cache exists as per rules set above, otherwise returns a cache instance identified by
     * cacheName
     */
     ...
     * @return null if no configuration exists as per rules set above, otherwise returns a cache instance identified by
     * cacheName
     */
