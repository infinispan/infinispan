...
/**
 * A layer of indirection around an {@link AtomicHashMap} to provide consistency and isolation for
 * concurrent readers while writes may also be going on.  The techniques used in this implementation
 * are very similar to the lock-free reader MVCC model used in the {@link
 * org.infinispan.container.entries.MVCCEntry} implementations for the core data container, which
 * closely follow software transactional memory approaches to dealing with concurrency. <br /><br />
 * Implementations of this class are rarely created on their own; {@link
 * AtomicHashMap#getProxy(AdvancedCache, Object)} should be used to retrieve an instance of this
 * proxy. <br /><br /> Typically proxies are only created by the {@link AtomicMapLookup} helper, and
 * would not be created by end-user code directly.
 *
...
public class AtomicHashMapProxy<K, V> extends AutoBatchSupport implements AtomicMap<K, V> {
    ...
    /**
     * Looks up the CacheEntry stored in transactional context corresponding to this AtomicMap.  If
     * this AtomicMap has yet to be touched by the current transaction, this method will return a
     * null.
     *
     * @return
     */
