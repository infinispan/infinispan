...
/**
 * A helper that locates or safely constructs and registers atomic maps with a given cache.  This
 * should be the <b>only</b> way AtomicMaps are created/retrieved, to prevent concurrent creation,
 * registration and possibly overwriting of such a map within the cache.
 *
...
public class AtomicMapLookup {
    ...
    /**
     * Retrieves an atomic map from a given cache, stored under a given key.  If an atomic map did
     ...
     * @param cache underlying cache
     * @param key key under which the atomic map exists
     * @param createIfAbsent if true, a new atomic map is created if one doesn't exist; otherwise
     * null is returned if the map didn't exist.
     * @param <MK> key param of the cache
     * @param <K> key param of the AtomicMap
     * @param <V> value param of the AtomicMap
     ...
     * @param createIfAbsent if true, a new atomic map is created if one doesn't exist; otherwise
     * null is returned if the map didn't exist.
     * @param fineGrained if true, and createIfAbsent is true then created atomic map will be fine
     * grained.
     * @return an AtomicMap, or null if one did not exist.
     */
    @SuppressWarnings("unchecked")
    private static <MK, K, V> Map<K, V> getMap(Cache<MK, ?> cache, MK key, boolean createIfAbsent,
        boolean fineGrained) {
    ...
    /**
     * Retrieves an atomic map from a given cache, stored under a given key, for reading only.  The
     * atomic map returned will not support updates, and if the map did not in fact exist, an empty
     * map is returned.
     *
     * @param cache underlying cache
     * @param key key under which the atomic map exists
