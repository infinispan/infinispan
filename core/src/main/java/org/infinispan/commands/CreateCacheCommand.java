...
public class CreateCacheCommand extends BaseRpcCommand {
   ...
   public Object perform(InvocationContext ctx) throws Throwable {
      Configuration c1 = cacheManager.getCacheConfiguration(cacheConfigurationName);
      Configuration tmpCacheConfiguration = null;
      if (c1 != null) {
         //user configured tmp cache               
         tmpCacheConfiguration = new ConfigurationBuilder().read(c1).build();
         log.debug("Using specified tmp cache configuration "+ cacheConfigurationName);
      } else {
         //our sensible default
         tmpCacheConfiguration = new ConfigurationBuilder().clustering().cacheMode(CacheMode.DIST_SYNC).sync()
                  .transaction().lockingMode(LockingMode.PESSIMISTIC).build();
         log.debug("Using default tmp cache configuration ");
      }
      cacheManager.defineConfiguration(cacheNameToCreate, tmpCacheConfiguration);
      cacheManager.getCache(cacheNameToCreate); // getCache starts the cache as well*/
      log.debug("Defined and started tmp cache " + cacheNameToCreate);
      return true;
   }
