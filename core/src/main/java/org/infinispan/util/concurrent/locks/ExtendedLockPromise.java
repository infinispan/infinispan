...
public interface ExtendedLockPromise extends LockPromise {
    ...
    *
    * @param cause the cancellation cause. The possible values are {@link LockState#DEADLOCKED} and
   ...
   /**
    * @param timeoutSupplier The {@link TimeoutException} to use when the lock acquisition times out.
    * @return A {@link CompletionStage} which is completed when the lock is acquired or when it fails to acquire.
    */
   CompletionStage<Void> toCompletionStage(Supplier<TimeoutException> timeoutSupplier);
}
