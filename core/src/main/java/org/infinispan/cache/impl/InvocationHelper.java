...
public class InvocationHelper implements TransactionResourceConverter {
   ...
   public <T> T invoke(InvocationContext context, GetKeyValueCommand command) {
      return (T) invoker.invokeGet(context, command);
   }

   @SuppressWarnings("unchecked")
   public <T> CompletableFuture<T> invokeAsync(InvocationContext context, GetKeyValueCommand command) {
      return (CompletableFuture<T>) invoker.invokeGetAsync(context, command);
   }

   @SuppressWarnings("unchecked")
   public <T> T invoke(InvocationContext context, GetCacheEntryCommand command) {
      return (T) invoker.invokeGetCacheEntry(context, command);
   }

   @SuppressWarnings("unchecked")
   public <T> CompletableFuture<T> invokeAsync(InvocationContext context, GetCacheEntryCommand command) {
