...
/**
 * This is a convenient base class for implementing a cache delegate. The only constructor takes a
 * {@link Cache} argument, to which each method call is delegated. One can extend this class and
 * override the method sub-set it is interested in. There is also an similar implementation for
 * {@link org.infinispan.AdvancedCache}: {@link org.infinispan.cache.impl.AbstractDelegatingAdvancedCache}.
 *
...
public abstract class AbstractDelegatingCache<K, V> implements Cache<K, V> {
    ...
    @Override
    public CompletableFuture<Void> putAllAsync(Map<? extends K, ? extends V> data, long lifespan,
        TimeUnit unit) {
    ...
    @Override
    public CompletableFuture<V> putIfAbsentAsync(K key, V value, long lifespan,
        TimeUnit lifespanUnit, long maxIdle, TimeUnit maxIdleUnit) {
    ...
    @Override
    public CompletableFuture<Boolean> replaceAsync(K key, V oldValue, V newValue, long lifespan,
        TimeUnit unit) {
    ...
    /**
     * Fully unwraps a given cache returning the base cache.  Will unwrap all
     * <b>AbstractDelegatingCache</b> wrappers.
     *
     * @param cache
     * @param <K>
     * @param <V>
