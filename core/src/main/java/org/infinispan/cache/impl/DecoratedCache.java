...
/**
 * A decorator to a cache, which can be built with a specific {@link ClassLoader} and a set of
 * {@link Flag}s.  This {@link ClassLoader} and set of {@link Flag}s will be applied to all cache
 * invocations made via this decorator.
 * <p/>
 * In addition to cleaner and more readable code, this approach offers a performance benefit to
 * using {@link AdvancedCache#withFlags(org.infinispan.context.Flag...)} API, thanks to internal
 * optimizations that can be made when the {@link Flag} set is unchanging.
 *
