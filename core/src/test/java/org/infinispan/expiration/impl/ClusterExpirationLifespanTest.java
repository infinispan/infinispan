...
public class ClusterExpirationLifespanTest extends MultipleCacheManagersTest {
   ...
   public Object[] factory() {
      return Stream.builder()
                   .add(new ClusterExpirationLifespanTest().cacheMode(CacheMode.DIST_SYNC).transactional(true).lockingMode(LockingMode.OPTIMISTIC))
                   .add(new ClusterExpirationLifespanTest().cacheMode(CacheMode.DIST_SYNC).transactional(true).lockingMode(LockingMode.PESSIMISTIC))
                   .add(new ClusterExpirationLifespanTest().cacheMode(CacheMode.DIST_SYNC).transactional(false))
                   .add(new ClusterExpirationLifespanTest().cacheMode(CacheMode.REPL_SYNC).transactional(true).lockingMode(LockingMode.OPTIMISTIC))
                   .add(new ClusterExpirationLifespanTest().cacheMode(CacheMode.REPL_SYNC).transactional(true).lockingMode(LockingMode.PESSIMISTIC))
                   .add(new ClusterExpirationLifespanTest().cacheMode(CacheMode.REPL_SYNC).transactional(false))
                   .add(new ClusterExpirationLifespanTest().cacheMode(CacheMode.SCATTERED_SYNC).transactional(false))
                   .build().toArray();
   }
...
//   }
   ...
   @Test(groups = "unstable", description = "Requires backport of ISPN-12337 and ISPN-12518")
