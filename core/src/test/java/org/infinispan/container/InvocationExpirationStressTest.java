...
import org.infinispan.executors.CompletableExecutorService;
...
public class InvocationExpirationStressTest extends SingleCacheManagerTest {
   ...
   private CompletableExecutorService executorService = new CompletableExecutorService(Executors.newFixedThreadPool(3));
   ...
   public void testExpiration() {
      cache.put("k", "v0");
      CompletableFuture<?> expirationFuture = executorService.submit(() -> {
         while (!terminated) {
            expirationManager.processExpiration();
            checkDataContainer();
         }
      });
      CompletableFuture<?> updateFuture = executorService.submit(() -> {
      ...
      CompletableFuture<?> timeout = executorService.submit(() -> {
         LockSupport.parkNanos(TimeUnit.SECONDS.toNanos(30));
         terminated = true;
      });
      CompletableFuture.anyOf(expirationFuture, updateFuture, timeout).join();
   }
