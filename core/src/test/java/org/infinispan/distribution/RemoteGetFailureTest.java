...
public class RemoteGetFailureTest extends MultipleCacheManagersTest {
   protected boolean staggered = true;
   private Object key;
   ...
   protected void createCacheManagers() throws Throwable {
      ...
      builder.clustering().sync().replTimeout(5, TimeUnit.SECONDS);
   ...
   public void testExceptionFromBothOwners(Method m) {
      ...
      cache(1).getAdvancedCache().addInterceptor(new FailingInterceptor(), 0);
      cache(2).getAdvancedCache().addInterceptor(new FailingInterceptor(), 0);
   ...
   public void testExceptionFromOneOwnerOtherTimeout(Method m) {
      ...
      cache(1).getAdvancedCache().addInterceptor(new FailingInterceptor(), 0);
      cache(2).getAdvancedCache().addInterceptor(new DelayingInterceptor(null, release), 0);
      ...
      try {
         ...
         long remoteTimeout = cache(0).getCacheConfiguration().clustering().sync().replTimeout();
   ...
   public void testBothOwnersSuspected(Method m) throws ExecutionException, InterruptedException {
      ...
      CountDownLatch release = new CountDownLatch(1);
      cache(1).getAdvancedCache().addInterceptor(new DelayingInterceptor(arrival, release), 0);
      cache(2).getAdvancedCache().addInterceptor(new DelayingInterceptor(arrival, release), 0);
   ...
   public void testOneOwnerSuspected(Method m) throws ExecutionException, InterruptedException {
      ...
      cache(1).getAdvancedCache().addInterceptor(new DelayingInterceptor(arrival, release1), 0);
      cache(2).getAdvancedCache().addInterceptor(new DelayingInterceptor(arrival, release2), 0);
   ...
   public void testOneOwnerSuspectedNoFilter(Method m) throws ExecutionException, InterruptedException {
      ...
      ClusteredGetCommand clusteredGet = new ClusteredGetCommand(key, cache(0).getName(), Collections.EMPTY_SET, false, null, AnyEquivalence.getInstance());
      final int timeout = 15;
      RpcOptions rpcOptions = new RpcOptions(timeout, TimeUnit.SECONDS, null, ResponseMode.WAIT_FOR_VALID_RESPONSE, true, DeliverOrder.NONE);
   ...
   public void testOneOwnerSuspectedOtherTimeout(Method m) throws ExecutionException, InterruptedException {
      ...
      Future<?> future = fork(() -> {
         ...
         assertTrue("Request did not wait for long enough: " + duration,
               duration >= cache(0).getCacheConfiguration().clustering().sync().replTimeout());
      });
      assertTrue(arrival.await(10, TimeUnit.SECONDS));
   ...
   private static class FailingInterceptor extends BaseCustomInterceptor {
      @Override
   ...
   private static class DelayingInterceptor extends BaseCustomInterceptor {
