...
public class CacheNotifierImplWithConverterTest extends CacheNotifierImplTest {
    ...
    protected void addListener() {
        ...
        Set<Class> filterAnnotations = Util.<Class>asSet(CacheEntryCreated.class, CacheEntryModified.class,
                CacheEntryRemoved.class, CacheEntryExpired.class);
        n.addFilteredListener(cl, null, (key, oldValue, oldMetadata, newValue, newMetadata, eventType) 
                -> "custom event for test (" + eventType.getType().name() + "," + key + "," + newValue + ")",
                filterAnnotations);
    }
