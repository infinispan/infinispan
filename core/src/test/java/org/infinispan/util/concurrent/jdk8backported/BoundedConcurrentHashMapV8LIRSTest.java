...
import org.infinispan.util.concurrent.jdk8backported.BoundedConcurrentHashMapV8.LIRSEvictionPolicy;
import org.infinispan.util.concurrent.jdk8backported.BoundedConcurrentHashMapV8.LIRSNode;
import org.infinispan.util.concurrent.jdk8backported.BoundedConcurrentHashMapV8.Recency;
 ...
 * @since 7.1
 */
@Test(groups = "functional", testName = "util.concurrent.jdk8backported.BoundedConcurrentHashMapV8LIRSTest")
public class BoundedConcurrentHashMapV8LIRSTest extends BoundedConcurrentHashMapV8BaseTest {
   ...
   private void verifyQueueContents(BoundedConcurrentHashMapV8<String, String> bchm, 
   ...
   public void testStateBUpdate() {
      BoundedConcurrentHashMapV8<String, String> map = initializeMapToPaper();
      //    Stack S     Queue Q
      //    B (LIR)       E
      //   E (HIR-res)
      //    A (LIR)
      ...
      // same (this can cause some premature LIRS promotions
      {
         ...
         assertEquals(BoundedConcurrentHashMapV8.NULL_VALUE, map.innerPeek(node.item.key));
   ...
   private BoundedConcurrentHashMapV8<String, String> initializeMapToPaper() {
      // The paper uses a Lirs = 2 & Lhrs = 1 (which we get by using a size of 3)
      // floor(3 * .95) = 2
      BoundedConcurrentHashMapV8<String, String> map = createMap(3, Eviction.LIRS);
      // Stack S    Queue Q
      //  B (LIR)
      map.put("B", "B");
      //    Stack S     Queue Q
