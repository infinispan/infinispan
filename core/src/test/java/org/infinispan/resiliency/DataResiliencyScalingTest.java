...
//      builder.persistence().addSoftIndexFileStore();
   ...
   public void testLeaveDroppedWhenCoordinatorStopping() throws Throwable {
      ...
      CountDownLatch coordinatorTopologyStopped = new CountDownLatch(1);

      // Spy on coordinator's ClusterTopologyManager
      ClusterTopologyManagerImpl spyCtm = (ClusterTopologyManagerImpl)
            spy(extractGlobalComponent(manager(0), ClusterTopologyManager.class));

      // Block non-coordinator graceful leaves for resiliency-cache until the coordinator's topology manager has stopped
      // This triggers the stage where the component registry is stopping in the coordinator, but the transport still active.
      doAnswer(ivk -> {
         Address leaver = ivk.getArgument(1);
         long timeout = ivk.getArgument(2);
         String cache = ivk.getArgument(0);
         if (!leaver.equals(address(0)) && timeout > 0 && CACHE_NAME.equals(cache)) {
            assertThat(coordinatorTopologyStopped.await(30, TimeUnit.SECONDS)).isTrue();
         }
         return ivk.callRealMethod();
      }).when(spyCtm).handleLeave(any(), any(), anyLong(), any());

      // Signal when stop() completes (clusterManagerStatus = STOPPING, transport still alive)
      doAnswer(ivk -> {
         ivk.callRealMethod();
         coordinatorTopologyStopped.countDown();
         return null;
      }).when(spyCtm).stop();

      replaceComponent(manager(0), ClusterTopologyManager.class, spyCtm, true);

      // Start all stops concurrently
      CompletableFuture<?>[] cfs = new CompletableFuture[firstSurvivor];
      for (int i = 0; i < firstSurvivor; i++) {
      ...
      // The coordinator should keep bouncing until landing on the last surviving nodes.
      CompletableFuture.allOf(cfs).get(60, TimeUnit.SECONDS);
      eventually(() -> {
         DistributionManager dm = extractComponent(survivingCache, DistributionManager.class);
         return !dm.isRehashInProgress()
               && dm.getCacheTopology().getMembersSet().size() == survivors;
      });

      // Even though coordinator was changing during stop, the data should still be present.
      // The leave commands should still be handled serially even when changing coordinators.
...
//         assertDataIntegrity(cache(i, CACHE_NAME));
   ...
   public void testStopInterrupted() throws Throwable {
      ...
      StateTransferTracker globalTracker = extractGlobalComponent(leavingManager, StateTransferTracker.class);
      StateTransferTracker.CacheStateTransferTracker spyCacheTracker = spy(globalTracker.forCache(CACHE_NAME));
      doAnswer(invocation -> {
   ...
   public void testGracefulLeaveWithRebalanceDisabled() throws Throwable {
      ...
      eventually(() -> {
         DistributionManager dm = extractComponent(cache(0, CACHE_NAME), DistributionManager.class);
   ...
   private void assertEventuallySingleCache(int index) {
      eventually(() -> {
         DistributionManager dm2 = extractComponent(cache(index, CACHE_NAME), DistributionManager.class);
