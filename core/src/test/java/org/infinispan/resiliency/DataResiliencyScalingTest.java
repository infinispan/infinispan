...
//      builder.persistence().addSoftIndexFileStore();
   ...
   public void testInterruptingProcedure() throws Throwable {
      waitForClusterToForm(CACHE_NAME);
      populateCluster();
      ...
      Thread t = new Thread(() -> {
         try {
            cache(CLUSTER_SIZE - 1, CACHE_NAME).stop(10, TimeUnit.SECONDS);
   ...
   public void testScalingDownContainerInterrupted() throws Throwable {
      waitForClusterToForm(CACHE_NAME);
      populateCluster();
      ...
      Thread t = new Thread(() -> {
         for (int i = CLUSTER_SIZE - 1; i > 0; i--) {
            DefaultCacheManager dcm = (DefaultCacheManager) cacheManagers.get(i);
            if (i == 1) {
               cp.trigger("should_interrupt");
               cp.awaitStrictAsync("after_interrupt", 5, TimeUnit.SECONDS, testExecutor())
                     .thenRun(() -> {
                        try {
                           assertThat(dcm.stopAllCaches(30, TimeUnit.SECONDS)).isTrue();
                        } catch (InterruptedException e) {
      ...
      t.start();
      cp.awaitStrict("should_interrupt", 5, TimeUnit.SECONDS);
      t.interrupt();
      cp.triggerForever("after_interrupt");
