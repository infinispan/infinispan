...
@Test(groups = "unit", testName = "configuration.parsing.XmlFileParsingTest")
   ...
   public void testParseAndConstructUnifiedXmlFile() throws IOException {
      withCacheManager(new CacheManagerCallable(
         ...
         public void call() {
            ...
            threadFactory = cm.getCacheManagerConfiguration().transport().threadPool().threadFactory();
            assertEquals("infinispan", threadFactory.threadGroup().getName());
            assertEquals("%G %i", threadFactory.threadNamePattern());
            assertEquals(5, threadFactory.initialPriority());
            BlockingThreadPoolExecutorFactory threadPool = cm.getCacheManagerConfiguration().transport().threadPool().threadPoolFactory();
