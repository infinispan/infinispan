...
public abstract class AbstractJCache<K, V> implements Cache<K, V> {
    ...
    public AbstractJCache(MutableConfiguration<K, V> configuration, CacheManager cacheManager,
    ...
    protected List<K> filterLoadAllKeys(Set<? extends K> keys, boolean replaceExistingValues,
        boolean cacheEvict) {
        if (trace)
    ...
    protected void loadAllFromJCacheLoader(Set<? extends K> keys, boolean replaceExistingValues,
    ...
    protected void removeCacheEntryListenerConfiguration(
        CacheEntryListenerConfiguration<K, V> listenerCfg) {
