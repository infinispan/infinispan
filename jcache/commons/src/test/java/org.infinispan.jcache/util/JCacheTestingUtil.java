...
public class JCacheTestingUtil {
   ...
   public static Cache getCache(CachingProvider provider, Class invoker, String fileName, String cacheName) {
      try {
         URI configUri = invoker.getClassLoader().getResource(fileName).toURI();
         CacheManager cacheManager = provider.getCacheManager(configUri, new TestClassLoader(Thread.currentThread().getContextClassLoader()));
         return cacheManager.getCache(cacheName);
      } catch (Exception e) {
         // Rethrow as ISE in order not to clutter tests with 'throws' clauses
         throw new IllegalStateException(e);
      }
   }
